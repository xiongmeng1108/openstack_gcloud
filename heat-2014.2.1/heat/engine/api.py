...
from heat.api.openstack.v1.g_cloud import *
import pytz
...
def format_stack(stack, preview=False):
    ...
    info = {
        ...
        api.STACK_CREATION_TIME: stack.created_time.replace(tzinfo=pytz.utc).astimezone
        ...
        #add by xm-20150605-for show stack
        #api.STACK_DESCRIPTION: stack.t[stack.t.DESCRIPTION],
        api.STACK_DESCRIPTION: stack.description,
        ...
        #add by xm-20150605-for show stack
        api.APP_NAME: stack.app_name,
        api.ENDUSER: stack.enduser,
        api.STACK_APPS_STYLE: stack.stack_apps_style,
        api.ISSCALER: stack.isscaler,
        api.TEMPLATE_ID: stack.template_id,
    }

    if not preview:
        update_info = {
            api.STACK_ACTION: stack.action or '',
            api.STACK_STATUS: change_status(stack.status, stack_action=stack.action, flag='resp') or '',
