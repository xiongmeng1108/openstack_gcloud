    ...
    def update_task(self, newstack, action=UPDATE, event=None):
        ...
        if action == self.UPDATE:
            ...
            oldstack = Stack(self.context, self.name, copy.deepcopy(self.t),
                             self.env,
                             #add by xm-20150603
                             enduser=self.enduser, stack_apps_style=self.stack_apps_style, isscaler=self.isscaler, description=self.description, app_name=self.app_name)
        backup_stack = self._backup_stack()  #这里生成数据库新记录backup（新记录的栈名字为“原名字+'*'”“XM2222222*”,bakcup=1）
        try:
            ...
            #add by xm-20150603-更新原有记录，必须保存以下字段
            self.enduser = newstack.enduser
            self.stack_apps_style = newstack.stack_apps_style
            self.isscaler = newstack.isscaler
            self.description = newstack.description
            self.app_name = newstack.app_name
            ...
            backup_stack.delete(None, backup=True)
