...
(EXACT_CAPACITY, CHANGE_IN_CAPACITY, PERCENT_CHANGE_IN_CAPACITY,
 SCALE_OUT_FIRST, SCALE_UP_FIRST, SCALE_OUT, SCALE_UP) = (
    'ExactCapacity', 'ChangeInCapacity', 'PercentChangeInCapacity',
    'scaleOutFirst', 'scaleUpFirst', 'scaleOut', 'scaleUp')
...
def _calculate_new_capacity(current, adjustment, adjustment_type,
    ...
    """
    if adjustment_type == CHANGE_IN_CAPACITY or adjustment_type == SCALE_OUT or adjustment_type == SCALE_OUT_FIRST:
...
class InstanceGroup(stack_resource.StackResource):
    ...
    def __init__(self, name, json_snippet, stack):
        ...
        """
        #just fo scale_up
        self.scale_up_cpu = json_snippet["Properties"]['resource']['properties'].get("core")
        self.scale_up_mem = json_snippet["Properties"]['resource']['properties'].get("memory")
    ...
    def get_instances(self):
        ...
        if self.nested():
            ...
            #resources = [resource for resource in self.nested().itervalues()]  # change by  zyk
    ...
    def _get_instance_definition(self):
        ...
        #add by zyk
        #props['createUserId'] = self.stack.enduser
        if vpc_zone_ids:
...
class AutoScalingGroup(InstanceGroup, cooldown.CooldownMixin):
    ...
    PROPERTIES = (
        ...
        FLOATING_NETWORKING, FLOATING_IP,
    ) = (
        'AvailabilityZones', 'LaunchConfigurationName', 'maxInst', 'minInst',
    ...
    properties_schema = {
        ...
        FLOATING_NETWORKING: properties.Schema(
            properties.Schema.STRING,
            _(' floating network.'),
    ...
    def handle_create(self):
        resc = self.properties[self.RESOURCE]
    ...
    def _scale_out(self, adjustment, adjustment_type):
        ...
        LOG.debug(_("scale out and vm counts %s") %new_capacity)
    ...
    def adjust(self, adjustment, adjustment_type=CHANGE_IN_CAPACITY):
        ...
        """
        LOG.debug(_("start to %s group ") %adjustment_type)
        ...
        if adjustment_type == CHANGE_IN_CAPACITY  or adjustment_type ==EXACT_CAPACITY \
                or adjustment_type == PERCENT_CHANGE_IN_CAPACITY or adjustment_type == SCALE_OUT:

            self._scale_out(adjustment, adjustment_type)

        if adjustment_type == SCALE_UP:
            self._scale_up(adjustment, adjustment_type)

        if adjustment_type == SCALE_OUT_FIRST:
            flag = self._scale_out(adjustment, adjustment_type)
            if flag:
                self._scale_up(adjustment, adjustment_type)

        if adjustment_type == SCALE_UP_FIRST:
            flag = self._scale_up(adjustment, adjustment_type)
            if flag:
                self._scale_out(adjustment, adjustment_type)
    ...
    def validate(self):
        ...
        if self.properties[self.MIN_SIZE] is not None and self.properties[self.MAX_SIZE] is not None:
...
class AutoScalingResourceGroup(AutoScalingGroup):
    ...
    PROPERTIES = (
        ...
        ROLLING_UPDATES, CPU_RESIZE, MEM_RESIZE, MAX_CPU, MAX_MEM,
        FLOATING_NETWORKING, FLOATING_IP,
    ) = (
        'resource', 'maxInst', 'minInst', 'cooldown', 'desired_capacity',
        'rolling_updates', 'cpu_resize', 'mem_resize', 'max_cpu', 'max_mem', 'floating_network',
        'floating_ip',
    )
    ...
    properties_schema = {
        ...
        CPU_RESIZE: properties.Schema(
            properties.Schema.INTEGER,
            _(' resize number of cpu.'),
            update_allowed=True
        ),
        MEM_RESIZE: properties.Schema(
            properties.Schema.INTEGER,
            _(' resize number of memory.'),
            update_allowed=True
        ),
        MAX_CPU: properties.Schema(
            properties.Schema.INTEGER,
            _(' max number of cpu.'),
            update_allowed=True
        ),
        MAX_MEM: properties.Schema(
            properties.Schema.INTEGER,
            _(' max memory of cpu.'),
            update_allowed=True
        ),
    ...
    def _get_instance_definition(self):
        rsrc = self.properties[self.RESOURCE]
        #rsrc.get('properties')['createUserId'] = self.stack.enduser
        return rsrc_defn.ResourceDefinition(None,
                                            rsrc['type'],
                                            rsrc.get('properties'),
                                            rsrc.get('metadata'))
...
def resource_mapping():
    return {
         'OS::Heat::G_CloudAutoScalingGroup': AutoScalingResourceGroup,
    }
