...
class Pool(neutron.NeutronResource):
    ...
    def handle_create(self):
        try:
            ...
            self.stack.error_codes.append('heat_server_task_00006')
            raise e
    ...
    def check_create_complete(self, data):
        ...
        if status == 'PENDING_CREATE':
            ...
            self.stack.error_codes.append('heat_server_task_00007')
            raise resource.ResourceUnknownStatus(
                resource_status=status,
...
class PoolMember(neutron.NeutronResource):
    ...
    PROPERTIES = (
        POOL_ID, ADDRESS, PROTOCOL_PORT, WEIGHT, ADMIN_STATE_UP, INSTANCE_ID,
    ) = (
        'pool_id', 'address', 'protocol_port', 'weight', 'admin_state_up', 'instance_id'
    )
