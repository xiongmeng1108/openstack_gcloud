...
class Volume(resource.Resource):
    ...
    def _soft_delete(self, backup=False):
        if self.resource_id is not None:
            try:
                vol = self.cinder().volumes.get(self.resource_id)
                if vol.status == 'in-use':
                ...
                while True:
                    yield
                    vol = self.cinder().volumes.get(self.resource_id)
                    if vol.status == 'trashed':
                        LOG.debug(_("volume %s is  trashed") %self.resource_id)
                        break
            except Exception as ex:
                self.client_plugin().ignore_not_found(ex)
    ...
    def handle_delete(self):
        delete_task = scheduler.TaskRunner(self._soft_delete)
        delete_task.start()
        return delete_task
        ...
        if self.properties[self.POOL_NAME] == "auto":
            ...
            arguments = {
             ...
             arguments = {
                'size': self.properties[self.SIZE],
                'availability_zone': self.properties[self.AVAILABILITY_ZONE],
                'scheduler_hints': {'pool_name': self.properties[self.POOL_NAME]}
            }

        if self.properties[self.VOLUME_TYPE] == "Local":
            arguments["scheduler_hints"] = {'host_name': self.properties[self.HOST_NAME]}

        #arguments["scheduler_hints"].update({'pool_name': self.properties[self.POOL_NAME]})
        #scheduler_hints
