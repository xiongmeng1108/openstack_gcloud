...
class EngineService(service.Service):
    ...
    #add by xm-20150603-for create_stack/update_stack
    def _validate_new_stack_by_app_name(self, cnxt, app_name):
        if app_name is None:
            raise exception.StackAppNameIsNone()
        if db_api.stack_get_by_app_name(cnxt, app_name):
            raise exception.StackExistsAppName(app_name=app_name)
    ...
    def _parse_template_and_validate_stack(self, cnxt, stack_name, template,
        ...
        #add by xm-20150603
        self._validate_new_stack_by_app_name(cnxt, args.get("app_name", None))
        ...
        stack = parser.Stack(cnxt, stack_name, tmpl, env,
                             owner_id=owner_id,
                             #add by xm-20150602
                             stack_apps_style=args.get("stack_apps_style", None),
                             isscaler=args.get("isscaler", None),
                             enduser=args.get("enduser_id", None),
                             description=args.get("description", None),
                             #add by xm-20150603
                             app_name=args.get("app_name", None),
                             #---end---
                             #add by zyk
                             template_id=args.get("template_id", None),
                             **common_params)
    ...
    def create_stack(self, cnxt, stack_name, template, params, files, args,
        ...
        def _stack_create(stack, task_id):
        ...
        stack.store()   #heat/engine/stack.py: class Stack line288

        self.thread_group_mgr.start_with_lock(cnxt, stack, self.engine_id,
                                              _stack_create, stack, task_id)

        return dict(stack.identifier())

    @request_context
    def update_stack(self, cnxt, stack_identity, template, params,
                     files, args, task_id=None):
        """
        ...
        #add by xm-20150603
        stack_app_name = args.get("app_name", None)
        #self._validate_new_stack_by_app_name(cnxt, stack_app_name)
        if stack_app_name is not None:
            common_params.setdefault("app_name", stack_app_name)

        #add by xm-20150603
        common_params.setdefault("enduser", args.get("enduser", db_stack.enduser))
        common_params.setdefault("stack_apps_style", args.get("stack_apps_style", db_stack.stack_apps_style))
        common_params.setdefault("isscaler", args.get("isscaler", db_stack.isscaler))
        common_params.setdefault("description", args.get("description", db_stack.description))
        ...
        updated_stack = parser.Stack(cnxt, stack_name, tmpl,
                                     #会利用common_params参数生成updated_stack
                                     env, **common_params)
        ...
        #line 104 def start_with_lock()
        th = self.thread_group_mgr.start_with_lock(cnxt, current_stack,
                                                   self.engine_id,
                                                   #经过line 104 def start_with_lock()，最后进入current_stack的update()方法运行
                                                   # (updated_stack和enent作为参数传入)：
                                                   # heat/engine/service.py/def update(self, newstack, event=None):
    ...
    @request_context
    def delete_stack(self, cnxt, stack_identity, task_id=None):
        """
    ...
    @request_context
    def stack_suspend(self, cnxt, stack_identity, task_id=None):
        '''
        Handle request to perform suspend action on a stack
        '''
        def _stack_suspend(stack, task_id):
            LOG.debug("suspending stack %s" % stack.name)
            stack.suspend(task_id)

        s = self._get_stack(cnxt, stack_identity)

        stack = parser.Stack.load(cnxt, stack=s)
        self.thread_group_mgr.start_with_lock(cnxt, stack, self.engine_id,
                                              _stack_suspend, stack, task_id)

    @request_context
    def stack_resume(self, cnxt, stack_identity, task_id=None):
        '''
        Handle request to perform a resume action on a stack
        '''
        def _stack_resume(stack, task_id):
            LOG.debug("resuming stack %s" % stack.name)
            stack.resume(task_id)

        s = self._get_stack(cnxt, stack_identity)

        stack = parser.Stack.load(cnxt, stack=s)
        self.thread_group_mgr.start_with_lock(cnxt, stack, self.engine_id,
                                              _stack_resume, stack, task_id)
