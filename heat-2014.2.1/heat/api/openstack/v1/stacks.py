...
from g_cloud import *
from heat.db.sqlalchemy import api as sqlalchemy_api
...
class StackController(object):
    ...
    def list_stacks_for_G_cloud(self, req, tenant_id):
        """
        return all stacks
        """
        #brk(host="10.10.12.21", port=49163)
        ...
        stacks = stack_list_from_db(req)
        ###alter by xm at 2015.9.2 --begin--
        #1: add total_stacks and total_scalar_stack in function format_stacks_reponse
        response = format_stacks_response(stacks)
        #2: commit the follow 3lines
        # total_stacks = sqlalchemy_api.stack_count_all_gcloud(req.context)
        # total_scalar_stacks = sqlalchemy_api.stack_count_all_gcloud(req.context, isscaler=True)
        # response.update({"total_stacks": total_stacks, "total_scalar_stack": total_scalar_stacks})
        ###---end---
        return response

    def list_stacks_report_for_G_cloud(self, req, tenant_id):
        """list all stacks report with stack_apps_style """
        response = data_list_from_db(req)
        return {"data": response, "success": True}

    def get_stack_name_by_stack_id_for_G_cloud(self, req, stack_id):
        """get the stack_name from stack_id """
        response = get_stack_name_by_stack_id(req, stack_id)
        return {"data": response, "success": True}

    def update_stack_for_G_cloud(self, req, tenant_id, stack_id, body):
        LOG.debug("update  stack for gcloud  %s " %body)
        validate_keys= set(["app_name", "enduser", "description", "stack_apps_style"])
        keys = [key for key in body.iterkeys()]
        if set(keys)-validate_keys:
            raise exc.HTTPBadRequest("request is not right")
        else:
            db_api.stack_update(req.context, stack_id, body)
