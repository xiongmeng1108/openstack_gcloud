...
def _convert_date_to_timestamp(date_str,is_begin=True):
    """
        if datestr is date,convert_date_to_timestamp;
        else raise error
        if is_begin==true,now one day begin time,else one day end time
        param:datestr
        param:is_begin
        return:timestampstr
    """
    if timeutils.is_valid_date(date_str)==False:
         msg = _(("time '%s' is not format '%s'")%(date_str,timeutils.DATE_FORMAT))
         LOG.error(msg)
         raise exc.HTTPBadRequest(msg)
    timestamp_str=None
    if is_begin:
        timestamp_str=date_str+"T00:00:00.000000"
    else:
         timestamp_str=date_str+"T23:59:59.999999"
    return timestamp_str

def validate_update_time_range(filters):
    """
     vailate create_time_first and crete_time_end,and
     convert date to timestamp
    """
    created_time_first=filters.get("create_time_first",None)
    created_time_end=filters.get("create_time_end",None)
    if created_time_first or created_time_end :
        if created_time_first:
            created_time_first=_convert_date_to_timestamp(created_time_first[0],is_begin=True)
        if  created_time_end:
            created_time_end=_convert_date_to_timestamp(created_time_end[0],is_begin=False)
        if created_time_first and created_time_end:
             if created_time_first>created_time_end:
                 msg = _(("created_time_first '%s' is  greater created_time_end '%s'")%(created_time_first,created_time_end))
                 LOG.error(msg)
                 raise exc.HTTPBadRequest(msg)
        if created_time_first:
           filters["create_time_first"]=created_time_first
        if created_time_end:
           filters["create_time_end"]=created_time_end
...
def get_offset(request):
    """Return marker, limit tuple from request.add by luoyibing 20150512
    :param request: offset
    """
    offset=  _get_offset_param(request)
    return offset
...
def _get_offset_param(request):
    """Extract integer offset from request or fail.add by luoyibing 20150512"""
    try:
        ...
        pass
    msg = _("offset must be an integer 0 or greater and not '%d'")
    raise exceptions.BadRequest(resource='offset', msg=msg)
...
class PaginationEmulatedHelper(PaginationHelper):
    ...
    def __init__(self, request, primary_key='id'):
        ...
        self.offset=get_offset(request)
    ...
    def update_args(self, args):
        ...
        if self.offset:
            args.update({'limit': self.limit, 'marker': self.marker,
                     'page_reverse': self.page_reverse,'offset':self.offset})#add by luoyibing 20150512 for offset
        else:
            args.update({'limit': self.limit, 'marker': self.marker,
