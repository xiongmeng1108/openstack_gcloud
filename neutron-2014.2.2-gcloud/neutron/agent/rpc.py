...
class PluginApi(n_rpc.RpcProxy):
    ...
    def get_qos_details(self, context, device, agent_id):
        return self.call(context,
                         self.make_msg('get_qos_details', device=device,agent_id=agent_id),
                         topic=self.topic)
    ...
    def tunnel_sync(self, context, tunnel_ip, tunnel_type=None):   #此处调用neutron-server的"tunnel_sync"消费者服务
        return self.call(context,
                         self.make_msg('tunnel_sync', tunnel_ip=tunnel_ip,
                                       tunnel_type=tunnel_type))     #返回结果如：{u'tunnels': [{u'ip_address': u'10.10.0.84'}, {u'ip_address': u'10.10.0.85'}, {u'ip_address': u'10.10.0.86'}, {u'ip_address': u'20.251.44.144'}]}
