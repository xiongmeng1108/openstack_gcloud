...
from gcloud  import client as g_client
...
class MetadataProxyHandler(object):
    OPTS = [
        ...
        cfg.StrOpt('user_data_server_type',
                   default='gcompute',
                   help=_("type of user data server"))
    ]
    ...
    def __call__(self, req):
        try:
            ...
            if instance_id:
                if self.conf.user_data_server_type == "gcompute":
                    return self._proxy_request_for_gcompute(instance_id, tenant_id, req)
                else:
    ...
    def _proxy_request_for_gcompute(self, instance_id, tenant_id, req):

        g_cloud_client = g_client.ComputeClient()
        LOG.debug("req patch is %s" %req.path_info)
        if req.path_info == "/2009-04-04/user-data":
            data = g_cloud_client.detail(instance_id)
            if data['userdata']:
                req.response.body = str(data['userdata'])
                LOG.debug("user data is %s" %data['userdata'])
            return req.response
        else:
           return webob.exc.HTTPNotFound()
