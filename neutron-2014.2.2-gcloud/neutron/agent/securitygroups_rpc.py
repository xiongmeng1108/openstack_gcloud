...
class SecurityGroupAgentRpcCallbackMixin(object):   #在ovs-neutron_agent的main函数初始化OVSNeutronAgent对象时作用
    ...
    def security_groups_member_updated(self, context, **kwargs):
        ...
        if not self.sg_agent:
            return self._security_groups_agent_not_set()
        self.sg_agent.security_groups_member_updated(security_groups)   #调用的就是下面class SecurityGroupAgentRpcMixin(object):的方法

    def security_groups_provider_updated(self, context, **kwargs):
        """Callback for security group provider update."""
        LOG.debug(_("Provider rule updated"))
        if not self.sg_agent:       #self.sg_agent: neutron.plugins.openvswitch.agent.ovs_neutron_agent.OVSSecurityGroupAgent
            return self._security_groups_agent_not_set()
        self.sg_agent.security_groups_provider_updated()   #调用的就是下面class SecurityGroupAgentRpcMixin(object):的方法


class SecurityGroupAgentRpcMixin(object):     #被neutron.plugins.openvswitch.agent.ovs_neutron_agent.OVSSecurityGroupAgent继承
    """A mix-in that enable SecurityGroup agent
    support in agent implementations.
    """
    ...
    def _security_group_updated(self, security_groups, attribute):
        ...
        #self.firewall.ports.values()的值（每一项对应一个虚拟机端口（如compute:43.111）的安全组规则：至少包括了network:dhcp端口）： [{u'status': u'ACTIVE', u'security_group_source_groups': [], u'binding:host_id': u'controller', u'name': u'', u'allowed_address_pairs': [], u'admin_state_up': True, u'network_id': u'139c7e8e-d3b7-4556-8e59-1e5822fa90f1', u'tenant_id': u'8bcbc26515234902a12a0185b2a64bdd', u'extra_dhcp_opts': [], u'security_group_rules': [{u'source_port_range_min': 67, u'direction': u'ingress', u'protocol': u'udp', u'ethertype': u'IPv4', u'port_range_max': 68, u'source_port_range_max': 67, u'source_ip_prefix': u'6.6.6.4', u'port_range_min': 68}, {u'source_port_range_min': 67, u'direction': u'ingress', u'protocol': u'udp', u'ethertype': u'IPv4', u'port_range_max': 68, u'source_port_range_max': 67, u'source_ip_prefix': u'6.6.6.27', u'port_range_min': 68}, {u'source_port_range_min': 67, u'direction': u'ingress', u'protocol': u'udp', u'ethertype': u'IPv4', u'port_range_max': 68, u'source_port_range_max': 67, u'source_ip_prefix': u'6.6.6.3', u'port_range_min': 68}], u'binding:vif_details': {u'port_filter': True, u'ovs_hybrid_plug': True}, u'binding:vif_type': u'ovs', u'device_owner': u'compute:43.111', u'mac_address': u'fa:16:3e:62:84:a3', u'device': u'ee622161-cff8-48a8-9f00-3c49c1620cc2', u'binding:profile': {}, u'binding:vnic_type': u'normal', u'fixed_ips': [u'6.6.6.24'], u'id': u'ee622161-cff8-48a8-9f00-3c49c1620cc2', u'security_groups': [u'31f899c8-a85b-4b65-ab33-cb3473913732'], u'device_id': u'0e0d384e-9ec6-45e8-bbbd-293147f10bfb'}]
