...
#test by xm 2015.7.30-for add security_groups rules of ingress to vm, as neutron-openvswi-i0f908f9f-1
SPOOF_FILTER_2 = 'spoof-filter_2'
...
class IptablesFirewallDriver(firewall.FirewallDriver):
    ...
    #test by xm 2015.7.30-for add security_groups rules of ingress to vm, as neutron-openvswi-i0f908f9f-1
    #add by xm-2015.7.30-for ip/mac bingding ingress
    def _setup_spoof_filter_chain_2(self, port, table, mac_ip_pairs, rules):
        if mac_ip_pairs:
            chain_name = self._port_chain_name(port, SPOOF_FILTER_2)
            table.add_chain(chain_name)
            for mac, ip in mac_ip_pairs:
                if ip is None:
                    # If fixed_ips is [] this rule will be added to the end
    ...
    #test by xm 2015.7.30-for add security_groups rules of ingress to vm, as neutron-openvswi-i0f908f9f-1
    def _spoofing_rule_2(self, port, ipv4_rules, ipv6_rules):
        mac_ipv4_pairs = []
        mac_ipv6_pairs = []
        ...
        self._setup_spoof_filter_chain_2(port, self.iptables.ipv4['filter'],
                                         mac_ipv4_pairs, ipv4_rules)
        self._setup_spoof_filter_chain_2(port, self.iptables.ipv6['filter'],
                                         mac_ipv6_pairs, ipv6_rules)
    ...
    def _port_arg(self, direction, protocol, port_range_min, port_range_max):
        ###alter by xm at 2015.9.2 for icmp protocol when using ECHO(ECHO Request,
        #  which type=0 [port_range_min = 0L],code=0)---begin---###
