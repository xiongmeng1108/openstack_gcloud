...
class OVSBridge(BaseOVS):
    ...
    def add_tunnel_port(self, port_name, remote_ip, local_ip,
        ...
        self.run_vsctl(
            vsctl_command)
        #vsctl_command=['--', '--may-exist', 'add-port', 'br-tun', u'gre-0a0a0055', '--', 'set', 'Interface', u'gre-0a0a0055', u'type=gre', 'options:df_default=true', u'options:remote_ip=10.10.0.85', 'options:local_ip=10.10.0.84', 'options:in_key=flow', 'options:out_key=flow']
    ...
    def get_port_tag_dict(self):
        ...
        return port_tag_dict
        #返回br-int上所有的{“端口:本地vlan”}(网卡端口，不包括br-int，patch-int等port)，如：{u'qr-77bec540-b3': 2, u'qvoee622161-cf': 2, u'tap9bf56d57-97': 1, u'qr-8f9a5cdd-3d': 1, u'patch-tun': [], u'tap6f3f6216-ef': 2, u'int-br-ex': [], u'ha-0e10bc14-99': 3}
    ...
    #add by xm-2015.5.5
    def set_interface_qos(self, interface, ingress, outgress):
        # 举例：interface == u'ha-0e10bc14-99'
        # ingress表示的是虚拟机下行带宽   outgress表示的是虚拟机上行带宽
        #pdb.set_trace() #by xm

        #up：上行(outgress)
        if outgress == None:  #表示不限速
            outgress = 0

        ingress_policing_rate = "ingress_policing_rate=%s" % outgress
        #ingress_policing_burst = "ingress_policing_burst=%s" % outgress
        args = ["set", "interface", interface, ingress_policing_rate]
        self.run_vsctl(args)

        #down：下行(ingress)
        #首先清除qos限速规则
        self.clear_interface_qos_ingress(interface)
        #然后判断是否qos限速，是则限速
        if ingress is not None:
            #create queue
            #neutron-server传过来的单位为kbps，需转换成bps：网上资料显示*1000而非1024；
            # 2015.8.31重新修改为*1024
            rate = int(ingress)*1024
            ingress = '%d' % rate
            config = "other-config:max-rate=%s" % ingress
            args = ["create", "queue", config]
            res = self.run_vsctl(args)
            queue = res.strip()

            #create qos
            args = ["create", "qos", 'type=linux-htb']
            res = self.run_vsctl(args)
            qos = res.strip()

            #add queue to qos
            config = "queues=0=%s" % queue
            args = ["set", "qos", qos, config]
            self.run_vsctl(args)

            #add qos to port
            config = "qos=%s" % qos
            args = ["set", "port", interface, config]
            self.run_vsctl(args)

    #清除qos上行限速
    def clear_interface_qos_outgress(self, port_name):
        ingress_policing_rate = "ingress_policing_rate=0"
        #ingress_policing_burst = "ingress_policing_burst=0"
        args = ["set", "interface", port_name, ingress_policing_rate]
        self.run_vsctl(args)

    #清除qos下行限速
    def clear_interface_qos_ingress(self, port_name):
        args = ['--format=json', '--', '--columns=qos', 'list', 'port', port_name]
        result = self.run_vsctl(args)
        if not result:
            return
        ...
        json_result = jsonutils.loads(result)
        try:
            qos = json_result['data'][0][0][1]  #when no qos,json_result['data'][0][0][1]=[]
        except Exception as e:
            LOG.warn(_("qosdebug: Unable to parse qos details of port : %s. Exception: %s"), port_name, e)
            return
        ...
        try:
            ...
            LOG.warn(_("qosdebug: Unable to parse queue details of qos : %s. Exception: %s"), qos, e)
            return
    ...
    #port_name对应的是br-int上端口的名称
    def clear_interface_qos(self, port_name):
        #清除qos上行限速
        self.clear_interface_qos_outgress(port_name)
        #清除qos下行限速
        self.clear_interface_qos_ingress(port_name)
...
class DeferredOVSBridge(object):
    ...
    def add_flow(self, **kwargs):
        self.action_flow_tuples.append(('add',
                                        kwargs))
        #self.action_flow_tuples=[('add', {'priority': 1, 'actions': 'resubmit(,3)', 'in_port': '2'}), ('mod', {'table': 22, 'dl_vlan': 1, 'actions': 'strip_vlan,set_tunnel:2,output:2'}), ('add', {'priority': 2, 'table': 20, 'dl_dst': u'fa:16:3e:4f:04:59', 'dl_vlan': 1, 'actions': 'strip_vlan,set_tunnel:2,output:2'})]
