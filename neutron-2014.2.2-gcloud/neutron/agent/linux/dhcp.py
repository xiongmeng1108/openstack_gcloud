...
class Dnsmasq(DhcpLocalProcess):
    ...
    @classmethod      #该修饰符可以无需实例化对象，直接"类名.方法"使用。@classmethod 仅仅适用于单独的，与类本身的数据结构无关函数，其实用了它的函数，与使用普通函数无异，甚至不能在参数里加入 self，如果要在其中使用类的数据结构，仍然需要将类实例化一次才可以，所以要小心使用。
    def should_enable_metadata(cls, conf, network):
        ...
        return any(isolated_subnets[subnet.id] for subnet in network.subnets)   #any()函数：只要有一个true(如不为空）则返回TRUE；all()函数：全部为true才返回True
