...
class LinuxInterfaceDriver(object):
    ...
    def init_l3(self, device_name, ip_cidrs, namespace=None,
                preserve_ips=[], gateway=None, extra_subnets=[], gw_cidr=None):
        ...
        # add new addresses
        for ip_cidr in ip_cidrs:  #ip_cidrs=[u'11.11.11.11/24']
            #add by xm for tz project 2015.12.10
            if ip_cidr == u'0':
                #LOG.debug("111111  : ip_cidr=%s", ip_cidr)
                continue
            #end 2015.12.10
        ...
        for ip_cidr, ip_version in previous.items():
            if ip_cidr not in preserve_ips:
                ...
                self.delete_conntrack_state(root_helper=self.root_helper,
                                            namespace=namespace,
                                            ip=ip_cidr)
        ###alter by xm for tz project at 2015.12.11
        # if gateway:
        #     device.route.add_gateway(gateway)
        #change as follow:
        #if gateway and ip_cidr != u'0':
        flag = False
        if gateway and ip_cidr == u'0':
            #LOG.debug(_("111111 INTERFACE 1 ip_cidr=%s"), ip_cidr)
            device.route.tz_add_gateway(gateway, gw_cidr)
            flag = True
        elif gateway and ip_cidr != u'0':
            #LOG.debug(_("111111 INTERFACE 2 ip_cidr=%s"), ip_cidr)
            device.route.add_gateway(gateway)
        # if gateway:
        #     LOG.debug(_("111111 INTERFACE 1 ip_cidr=%s"),ip_cidr)
        #     device.route.tz_add_gateway(gateway, gw_cidr)
        ...
        ### alter by xm for gc project at 2015.12.18
        #before is:
