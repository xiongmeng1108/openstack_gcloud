...
class IpRouteCommand(IpDeviceCommandBase):
    ...
    def tz_add_gateway(self, gateway, gw_cidr=None, metric=None, table=None):
        #ip netns exec qrouter-b90ba510-c348-4622-b4ea-f22042c39d88 ip route add  11.12.13.0/24 dev qg-1d68cee6-0b
        #accord to def add_gateway() in line 388
        args =['add', gw_cidr, 'dev', self.name]
        result = self._as_root(*args)
        LOG.debug("111111 result=%d", result)
        #set default route
        args = ['replace', 'default', 'via', gateway]
        if metric:
            args += ['metric', metric]
