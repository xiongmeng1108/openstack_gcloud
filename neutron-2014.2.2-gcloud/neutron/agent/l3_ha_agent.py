...
class AgentMixin(object):
    ...
    def _add_default_gw_for_ha_router_tz_other_way(self, ri, gateway=None, interface_name=None, gw_cidr=None):
        instance = ri.keepalived_manager.config.get_instance(ri.ha_vr_id)
        instance = ri.keepalived_manager.config.get_instance(ri.ha_vr_id)
        for vip in instance.vips:
            if vip.interface_name == interface_name:
                return
        # have no vip belong to gw_cidr in qg-xxxx of this router, delete the virtual_routes
        instance.virtual_routes = [vroute for vroute in instance.virtual_routes
