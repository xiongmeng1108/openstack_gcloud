...
class DhcpAgent(manager.Manager):
    ...
    def call_driver(self, action, network, **action_kwargs):
        ...
        try:
            ...
            getattr(driver, action)(**action_kwargs)    #此处启动dnamasq进程
            return True
    ...
    def configure_dhcp_for_network(self, network):
        ...
        for subnet in network.subnets:
            if subnet.enable_dhcp:
                if self.call_driver('enable', network):   #此处启动dnsmasq进程
        ...
        if enable_metadata and dhcp_network_enabled:
            for subnet in network.subnets:
                if subnet.ip_version == 4 and subnet.enable_dhcp:
                    self.enable_isolated_metadata_proxy(network)  #此处启动网络的neutron-ns-metadata-proxy进程
                    break
    ...
    def enable_isolated_metadata_proxy(self, network):
        ...
        def callback(pid_file):
            metadata_proxy_socket = cfg.CONF.metadata_proxy_socket   #cfg.CONF.metadata_proxy_socket=/var/lib/neutron/metadata_proxy
        ...
        pm = external_process.ProcessManager(
            ...
            self.root_helper,
            network.namespace)
        pm.enable(callback)   #启动该网络的neutron-ns-metadata-proxy进程：/usr/bin/python /bin/neutron-ns-metadata-proxy --pid_file=/var/lib/neutron/external/pids/794fee6b-0cb9-438b-99f8-351efa146236.pid --metadata_proxy_socket=/var/lib/neutron/metadata_proxy --network_id=794fee6b-0cb9-438b-99f8-351efa146236 --state_path=/var/lib/neutron --metadata_port=80 --debug --verbose --log-file=neutron-ns-metadata-prox-794fee6b-0cb9-438b-99f8-351efa146236.log --log-dir=/var/log/neutron
