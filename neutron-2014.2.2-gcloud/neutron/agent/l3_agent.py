...
from neutron.agent.linux import tc_lib_new
...
class RouterInfo(l3_ha_agent.RouterMixin):
    ...
    def __init__(self, router_id, root_helper, use_namespaces, router,
        ...
        #add floating ip qoss var
        self.floating_ip_qoss=[] #record qoss cache
        self.first = True  #first must get qoss from tc filter
...
class L3NATAgent(firewall_l3_agent.FWaaSL3AgentRpcCallback,
    ...
    OPTS = [
        ...
        cfg.StrOpt('metadata_proxy_socket',
                   ...
                   help=_('Location of Metadata Proxy UNIX domain '
                          'socket')),
        cfg.IntOpt('tc_classid_min',
                   default=10,
                   help=_("tc classid range.")),
        cfg.IntOpt('tc_classid_max',
                   default=30,
                   help=_("tc classid range.")),
        cfg.BoolOpt('qos_limit',
                    default=True,
                    help=_("Agent should implement routers with qos_limit")),
        cfg.StrOpt('default_limit',
                   default='1kbit',
                   help=_("tc default limit .")),
        cfg.BoolOpt('ingress_limit',
                    default=True,
                    help=_("Agent should implement routers with ingress_limit")),
        cfg.StrOpt('ingress_size',
                   default='375000kbit',
                   help=_("tc default ingress limit ."))
    ]
    ...
    def _set_subnet_info(self, port):
        ips = port['fixed_ips']
        if not ips:
            ###alter by xm for tz at 2015.12.10
            #LOG.debug(_("111111 l3_agent.py line 878"))
            port['ip_cidr'] = u'0'
            return
    ...
    def process_router(self, ri):
        LOG.debug("process_router begin:1  route_id %s"%(ri.router_id))
        # TODO(mrsmith) - we shouldn't need to check here
        if 'distributed' not in ri.router:
            ri.router['distributed'] = False
        ri.iptables_manager.defer_apply_on()
        ex_gw_port = self._get_ex_gw_port(ri)
        LOG.debug("process_router begin:2  ex_gw_port=%s"%(ex_gw_port))
        LOG.debug("process_router begin:3 ri=%s"%(ri.__dict__))
        ...
        for p in new_ports:
            ...
            self.internal_network_added(ri, p) #br-int上增加内网接口并添加到路由命名空间
        ...
        if ex_gw_port:
            ...
            LOG.debug(_('2016.1.5: ex_gw_port=%s  ri.ex_gw_port=%s'), ex_gw_port, ri.ex_gw_port)
            if not ri.ex_gw_port:
                #此处ex_gw_port= ri.get("gw_port")  与 ri.ex_gw_port(line 1028)不同
                self.external_gateway_added(ri, ex_gw_port, interface_name)
            ###alter for tz project at 2016.1.5
            # in order to add default rule each time in qrouter-xxx ns when route update
            #before is:
        ...
        try:
            if ex_gw_port:
                ...
                #/usr/lib/python2.7/site-packages/neutron/agent/l3_agent.py(1160)process_router_floating_ip_addresses()
        ...
        if ex_gw_port:
            self._update_qoss(ri,ex_gw_port)
            ###add by xm for gcloud_nat 2015.12.17
            #LOG.debug(_("111111 l3_agent.py going to create gcloud_nat! 3"))
            self.process_router_gcloud_nat_rules(ri, ex_gw_port)
            ri.iptables_manager.defer_apply_off()
            ###end

        if ri.is_ha:
            ###add for tz gcloud_nat 2016.1.7
            if ex_gw_port:
                self._add_default_gw_for_ha_router_tz_other_way(ri, interface_name=interface_name,
                                                gateway=ex_gw_port['subnet'].get('gateway_ip'),
                                                gw_cidr=ex_gw_port['subnet'].get("cidr",None))
            ###end 2016.1.7
            if ri.ha_port:
                ri.spawn_keepalived()
            else:
    ...
    def _update_qoss(self,ri,ex_gw_port):
          LOG.debug("_update_qoss in called")
          devname=self.get_external_device_name(ex_gw_port["id"])
          nsname=ri.ns_name
          qos_limit=tc_lib_new.QosLimit()
          if not self.conf.qos_limit:
              if ri.first:
                 qos_limit.del_qdisc_root(nsname,devname)
                 qos_limit.delete_ingress_qdisc(nsname,devname)
                 qos_limit.delete_router_tc_json_files(ri.router_id)
                 ri.first=False
              return
          if ri.first:
                if self.conf.ingress_limit:
                   qos_limit.add_ingress_limit(nsname,devname,ingress_size=self.conf.ingress_size)
                else:
                    qos_limit.delete_ingress_qdisc(nsname,devname)
                #{"rule_id3":{"max_rate":90,"class_id":10,"src_ips":["192.168.10.1","192.168.20.1"]},*****}
                ri.floating_ip_qoss=qos_limit.init_router_qos_cache(ri.router_id,nsname,devname,
                                                                    self.conf.tc_classid_min,self.conf.tc_classid_max,self.conf.default_limit)
                ri.first=False
          floating_cache_qoss= ri.floating_ip_qoss
          LOG.debug("floating_cache_qoss %r" %(floating_cache_qoss))
          #new_floating_qoss {"rule_id5":{"max_rate":2048,"src_ips":["11.168.10.1","11.168.20.1","11.0.0.1"]},****}
          new_floating_qoss=ri.router.get(l3_constants.FLOATING_IP_QOS_KEY, {})
          LOG.debug("new_floating_qoss %r" %(new_floating_qoss))
          qos_limit.update_qos_binds(ri.router_id,nsname,devname,new_floating_qoss,floating_cache_qoss,min_classid=self.conf.tc_classid_min,max_classid=self.conf.tc_classid_max)
          LOG.debug("update end,floating_cache_qoss %r" %(floating_cache_qoss))

    # def _update_qoss(self,ri,ex_gw_port):
    #     LOG.debug("_update_qoss in call")
    #     tc_operation=tc_lib.TC_Operation()
    #     interface_name = self.get_external_device_name(ex_gw_port["id"])
    #     if self.conf.qos_limit is False:#not set qos limit
    #         if ri.first:
    #             tc_operation.del_qdisc_root(ri.ns_name,interface_name)
    #             ri.first=False
    #         return
    #     if ri.first:
    #         tc_operation.add_root_and_class(ri.ns_name,interface_name,self.conf.default_limit)
    #         ri.floating_ip_qoss=tc_operation.get_class_and_flow(ri.ns_name,interface_name,self.conf.tc_classid_min,self.conf.tc_classid_max)
    #         ri.first=False
    #
    #     exsit_floatingip_qoss=ri.floating_ip_qoss
    #     current_floatingip_qoss=ri.router.get(l3_constants.FLOATING_IP_QOS_KEY, [])
    #
    #
    #     exist_qos_ids=set([qos['src_ip'] for qos in ri.floating_ip_qoss])
    #     current_qos_ids=set([qos['src_ip'] for qos in current_floatingip_qoss])
    #     new_qoss=  [qos for qos  in current_floatingip_qoss if
    #                  qos['src_ip'] in current_qos_ids and
    #                  qos['src_ip'] not in exist_qos_ids]
    #     old_qoss= [qos for qos in exsit_floatingip_qoss if
    #                 qos['src_ip'] not in current_qos_ids]
    #
    #     same_qos_ids=exist_qos_ids&current_qos_ids
    #     LOG.debug(" same_qos_ids %r,exist_qos_ids %r"%(same_qos_ids,exist_qos_ids))
    #     update_qoss=[]
    #     update_after_qoss=[]
    #     for  qos_id in same_qos_ids:
    #             current_qos=exist_qos=None
    #             for qos  in current_floatingip_qoss:
    #                 if qos['src_ip'] == qos_id:
    #                     current_qos=qos
    #                     break
    #             for qos  in exsit_floatingip_qoss:
    #                 if qos['src_ip'] == qos_id:
    #                     exist_qos=qos
    #                     break
    #             if current_qos.get("max_rate")!=exist_qos.get("max_rate"):
    #                 LOG.debug(" exist_qos %r current_qos %r"%(exist_qos,current_qos))
    #                 update_qoss.append(exist_qos)
    #                 update_after_qoss.append(current_qos)
    #
    #
    #      #add new qoss
    #
    #     for qos in new_qoss:
    #         LOG.debug(" add qos %r"%(qos))
    #         interface_name = self.get_external_device_name(ex_gw_port["id"])
    #         #add_class_and_flow(self,nsname = None,devname=None, classid=None,max_rate=None,src_ip=None):
    #         #generate qos class_id
    #         qos["class_id"]=self._get_qos_classid(ri)
    #         tc_operation.add_class_and_flow(ri.ns_name,interface_name,"1:"+str(qos["class_id"]),str(qos["max_rate"])+"kbit",qos["src_ip"])
    #         ri.floating_ip_qoss.append(qos)
    #         # add new_qoss to exist_qos
    #     #update qos
    #     index=0
    #     for qos in update_qoss:
    #
    #         update_after_qos=update_after_qoss[index]
    #         LOG.debug(" update oldqos %r,newqos %r"%(qos,update_after_qos))
    #         interface_name = self.get_external_device_name(ex_gw_port["id"])
    #         # def add_or_update_class(self,nsname=None,devname=None,parent_classid=None,curent_classid=None,rate=None,ceil=None ):
    #         tc_operation.add_or_update_class(ri.ns_name,interface_name,"1:1","1:"+str(qos["class_id"]),str(update_after_qos["max_rate"])+"kbit",str(update_after_qos["max_rate"])+"kbit")
    #        # update exist_qos
    #         qos["max_rate"]=update_after_qos["max_rate"]
    #         #ri.floating_ip_qoss.remove(qos)
    #        # ri.floating_ip_qoss.append(update_after_qos)
    #         index=index+1
    #     #delete old qoss
    #     for qos in old_qoss:
    #         LOG.debug(" delete qos %r"%(qos))
    #         interface_name = self.get_external_device_name(ex_gw_port["id"])
    #         #def del_flow_and_class(self,nsname = None,devname=None, classid=None):
    #         tc_operation.del_flow_and_class(ri.ns_name,interface_name,"1:"+str(qos["class_id"]))
    #        #delete exist_qos
    #         ri.floating_ip_qoss.remove(qos)
    # def _get_qos_classid(self,ri):
    #     exist_qos_class_ids=set([qos['class_id'] for qos in ri.floating_ip_qoss])
    #     classids_range=range(self.conf.tc_classid_min,self.conf.tc_classid_max+1)
    ...
    def process_router_gcloud_nat_rules(self, ri, ex_gw_port):
        """Configure NAT rules for the router's gcloud_nats.

        Configures iptables rules for the gcloud_nats of the given router
        """
        #LOG.debug(_("111111  AAAgent process_router_gcloud_nat_rules ri=%s"), ri.__dict__)
        # Clear out all iptables rules for gcloud_nats
        ri.iptables_manager.ipv4['nat'].clear_rules_by_tag('gcloud_nat')
        ri.iptables_manager.ipv4['filter'].clear_rules_by_tag('gcloud_nat')
        #LOG.debug(_("111111  AAAgent success delete gcloud_nat"))
        interface_name = self.get_external_device_name(ex_gw_port["id"])
        gcloud_nats = ri.router.get(l3_constants.GCLOUD_NAT_KEY, [])
        #LOG.debug(_("111111  AAAgent gcloud_nats=%s"), gcloud_nats)
        for gcloud_nat in gcloud_nats:
            for chain, rule in self.gcloud_nat_rules_nat(gcloud_nat):
                ri.iptables_manager.ipv4['nat'].add_rule(chain, rule, tag='gcloud_nat')
            for chain, rule in self.gcloud_nat_rules_filter(gcloud_nat, interface_name):
                ri.iptables_manager.ipv4['filter'].add_rule(chain, rule, tag='gcloud_nat')
    ...
    def _add_floating_ip(self, ri, fip, interface_name, device):
        ...
        ip_cidr = str(fip_ip) + "/" + fip['cidr']
        global FLOATING_IP_CIDR_SUFFIX
        FLOATING_IP_CIDR_SUFFIX = "/" + fip['cidr']
        LOG.debug(_("2016.1.4 fip_ip=%s  ip_cidr=%s"),  fip_ip, ip_cidr)
    ...
    def process_router_floating_ip_addresses(self, ri, ex_gw_port):
        ...
        ###add by xm for tz gcloud_nat 2016.1.7
        LOG.debug(_("floatingip_cidrs existing_cidrs = %s %s"), existing_cidrs, ex_gw_port.get("ip_cidr",'0'))
        existing_cidrs = existing_cidrs - set([ex_gw_port.get("ip_cidr","0")])
        LOG.debug(_("floatingip_cidrs existing_cidrs = %s"), existing_cidrs)
        ###end 2016.1.7
        new_cidrs = set()

        # Loop once to ensure that floating ips are configured.
        LOG.debug(_("floatingip_cidrs floating_ips = %s FLOATING_IP_CIDR_SUFFIX=%s"), floating_ips, FLOATING_IP_CIDR_SUFFIX)
        for fip in floating_ips:
            fip_ip = fip['floating_ip_address']
            ip_cidr = str(fip_ip) + FLOATING_IP_CIDR_SUFFIX
        ...
        LOG.debug(_("floatingip_cidrs new_cidrs = %s"), new_cidrs)
    ...
    def external_gateway_added(self, ri, ex_gw_port, interface_name):
        ...
        #此处如果配置neutron-server的neutron.conf的allow_gcloud_nat=True，且没有在路由上创建gcloud_nat
        #那么此时路由网关接口对应的端口没有分配网关ip, 此时　ex_gw_port['ip_cidr']=u'0' interface_name为路由ns命令空间里网关接口qg-xxxxx
        ...
        if ri.is_ha:
            ...
            #keepalived.conf配置有: 0 dev qg-xxxx配置项
            # virtual_ipaddress_excluded {
            #0 dev qg-2eaaf59e-75
            #10.0.0.1/24 dev qr-c635c9f7-d9
            #20.251.36.208/32 dev qg-2eaaf59e-75
            #20.251.36.209/32 dev qg-2eaaf59e-75
            #20.251.36.211/32 dev qg-2eaaf59e-75
            #}
    ...
    def _external_gateway_added(self, ri, ex_gw_port, interface_name,
        ...
        if not ri.is_ha:
            #LOG.debug("111111 215:ex_gw_port=%s ex_gw_port['ip_cidr']=%s", ex_gw_port, ex_gw_port['ip_cidr'])

            self.driver.init_l3(
                interface_name, [ex_gw_port['ip_cidr']], namespace=ns_name,
                gateway=ex_gw_port['subnet'].get('gateway_ip'),
                extra_subnets=ex_gw_port.get('extra_subnets', []),
                preserve_ips=preserve_ips, gw_cidr=ex_gw_port['subnet'].get("cidr",None))
    ...
    def gcloud_nat_rules_nat(self, nat):
        destination ='%s:%s' %  (nat["int_ip"],  nat["int_port"])
        return [('PREROUTING', '-d %s -p tcp -m tcp --dport %s -j DNAT  --to-destination %s' %
                 (nat["ext_ip"], nat["ext_port"], destination)),
                ('POSTROUTING', '-s %s -d %s -p tcp -m tcp --dport %s -j SNAT --to-source %s' %
                 (nat["vif_subnet_cidr"], nat["int_ip"], nat["int_port"], nat["vif_subnet_gateway"]))
                ]

    def gcloud_nat_rules_filter(self, nat, devname):
        return [('INPUT', '-d %s -p tcp -m tcp --dport %s -i %s -j ACCEPT' %
                (nat["int_ip"], nat["int_port"], devname))]
        ...
        for rp, update in self._queue.each_update_to_next_router():
            ...
            if update.action != DELETE_ROUTER and not router:
                try:
                    ...
                    LOG.debug("get_routers begin")
                    ##此处RPC从neutron-server获取router相关信息，待调试
                    routers = self.plugin_rpc.get_routers(self.context,
                                                          [update.id])
                    LOG.debug("get_routers end %r" %(routers))
                except Exception:
    ...
    def _add_default_gw_for_ha_router_tz(self, ri, gateway=None, interface_name=None, gw_cidr=None):
        # ip route add  11.12.13.0/24 dev qg-1d68cee6-0b
        if not gateway or not interface_name or not gw_cidr:
            LOG.debug(_("something error! gateway=%s, interface_name=%s, gw_cidr=%s"), gateway, interface_name, gw_cidr)
            return
        rt_id = ri.ns_name[8:]
        file_path = "/var/lib/neutron/ha_confs/%s/state" % rt_id
        LOG.debug(_("2016.1.5 file_path=%s"), file_path)
        try:
            file_object = open(file_path)
        except Exception:
            LOG.error(_("2016.1.5 no such file=%s exist!"), file_path)
        try:
            state = file_object.readline()
        finally:
            file_object.close()
        LOG.debug(_("_add_default_gw_for_ha_router_tz gateway=%s interface_name=%s gw_cidr=%s"), gateway, interface_name, gw_cidr)
        if state != 'master':
            LOG.debug(_("ha_confs/xxxx/state=%s going to delete gw route rule"), state)
            cmd1 = ["ip", 'route', 'del', gw_cidr, 'dev', interface_name]
            cmd2 = ["ip", 'route', 'del', 'default', 'via', gateway, 'dev', interface_name]
        else:
            cmd1 = ["ip", 'route', 'replace', gw_cidr, 'dev', interface_name]
            cmd2 = ["ip", 'route', 'replace', 'default', 'via', gateway, 'dev', interface_name]

        ip_wrapper = ip_lib.IPWrapper(self.root_helper, namespace=ri.ns_name)
        #LOG.debug(_("111111 _add_default_gw_for_ha_router_tz add default route start cmd1=%s cmd2=%s"), cmd1, cmd2)
        ip_wrapper.netns.execute(cmd1, check_exit_code=False)
        ip_wrapper.netns.execute(cmd2, check_exit_code=False)
        #LOG.debug(_("111111 _add_default_gw_for_ha_router_tz end"))

    def routes_updated(self, ri):
        new_routes = ri.router['routes']   #new_routes才是我们利用ri.__dic__打印看到的路由
        #LOG.debug(_("111111 11 route_updated new_routes=%s old_routes=%s"), new_routes, ri.routes)
        if ri.is_ha:
            self._process_virtual_routes(ri, new_routes)
            return

        old_routes = ri.routes
        #LOG.debug(_("111111 22 route_updated new_routes=%s old_routes=%s"), new_routes, old_routes)
