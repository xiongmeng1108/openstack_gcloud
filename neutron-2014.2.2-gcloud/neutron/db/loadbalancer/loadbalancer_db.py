...
class Member(model_base.BASEV2, models_v2.HasId,models_v2.HasCreateTime, models_v2.HasTenant,
             models_v2.HasStatusDescription):
...
class Pool(model_base.BASEV2, models_v2.HasId, models_v2.HasCreateTime,models_v2.HasTenant,
           models_v2.HasStatusDescription):
...
class HealthMonitor(model_base.BASEV2, models_v2.HasId, models_v2.HasCreateTime, models_v2.HasTenant):
...
class LoadBalancerPluginDb(loadbalancer.LoadBalancerPluginBase,
    ...
    def delete_vip(self, context, id):
        with context.session.begin(subtransactions=True):
            ...
            ###add by xm at 2015.9.7 for bug 6080 ---begin---
            floatingips=[""]
            l3plugin = manager.NeutronManager.get_service_plugins().get(service_constants.L3_ROUTER_NAT)
            if l3plugin:
                try:
                    l3plugin.delete_lb_vip_for_gcloud7(context, vip.port.id, floatingips=floatingips)
                except n_exc.Conflict:
                    raise loadbalancer.VipAssociateFloatingipExists(port_id=vip.port.id, floating_ip=floatingips[0])
            else:
                  LOG.error(_('No plugin for L3 routing registered to handle '
                                    'router scheduling'))
            ###add by xm at 2015.9.7 for bug 6080 ---end---
        ...
        # l3plugin = manager.NeutronManager.get_service_plugins().get(service_constants.L3_ROUTER_NAT)
        # if l3plugin:
        #      l3plugin.delete_floatingips_for_gcloud7(context, vip.port.id)
        # else:
        #       LOG.error(_('No plugin for L3 routing registered to handle '
        #                         'router scheduling'))
        # ###add by xm at 2015.9.7 for bug 6080 ---end---
    ...
    def get_vips_count(self, context, filters=None):
        return self._get_collection_count(context, Vip,
                                          filters=filters)

    def get_pools_count(self, context, filters=None):
        return self._get_collection_count(context, Pool,
                                          filters=filters)

    def get_members_count(self, context, filters=None):
        if 'resource_name' in filters:
            session = db.get_session()
            pool_ids = []
            if 'pool_id' in filters:    # 查询条件中即有"resource_name"也有"pool_id"
                pool_ids_2 = filters["pool_id"]
                for name in filters["resource_name"]:
                    with session.begin():
                        query = session.query(Pool).filter(Pool.name == name)
                        pools = query.all()
                        if pools:
                            for pool in pools:
                                if pool["id"] in pool_ids_2:
                                    pool_ids.append(pool["id"])
            else:  # 查询条件中只有"resource_name"没有"pool_id"
                for name in filters["resource_name"]:
                    with session.begin():
                        query = session.query(Pool).filter(Pool.name == name)
                        pools = query.all()
                        if pools:
                            for pool in pools:
                                pool_ids.append(pool["id"])
            del filters['resource_name']
            filters.update({"pool_id": pool_ids})
        return self._get_collection_count(context, Member,
                                          filters=filters)

    def get_health_monitors_count(self, context, filters=None):
        #add by xm-20150608-for filter by pool_id
        self._get_health_monitors_by_pool_id(context, filters)
        return self._get_collection_count(context, HealthMonitor,
                                          filters=filters)
    ...
    def get_vips(self, context, filters=None, fields=None, sorts=None, limit=None, marker=None, page_reverse=None, offset=None):
        return self._get_collection(context, Vip,
                                    self._make_vip_dict,
                                    filters=filters, fields=fields,
                                    sorts=sorts,
    ...
    def _get_vip_attr_by_vip_id(self, vip_id):
        session = db.get_session()
        with session.begin():
            vip = session.query(Vip).filter(id=pool["vip_id"]).first()
            if vip:
                vip_attr = {"vip_name": vip.name, "port_id": vip.port_id}
                return vip_attr
        return {}

    def _get_pool_attr(self, pool):
        pool_attr={}
        session = db.get_session()
        # with session.begin():
        #     vip = session.query(Vip).filter(Vip.id == pool["vip_id"]).first()
        #     if vip:
        #         pool_attr.update({"vip_name": vip.name, "port_id": vip.port_id})
        #     else:
        #         pool_attr.update({"vip_name": "", "port_id": ""})
        #Pool实体类（neutron/db/loadbalancer/loadbalancer_db.py）中关联了Vip实体类，详见：vip = orm.relationship(Vip, backref='pool')
        if pool["vip"]:
            pool_attr.update({"vip_name": pool["vip"].name, "port_id": pool["vip"].port_id})
            #add by xm-2015.7.14  BUG#5409：为get_pools添加浮动IP字段
            with session.begin():
                floatingip = session.query(l3_db.FloatingIP).filter(l3_db.FloatingIP.fixed_port_id == pool["vip"].port_id).first()
                if floatingip:
                    pool_attr.update({"floating_ip_address": floatingip.floating_ip_address})
                else:
                    pool_attr.update({"floating_ip_address": ""})
        else:
            pool_attr.update({"vip_name": "", "port_id": ""})
            #add by xm-2015.7.14  BUG#5409：为get_pools添加浮动IP字段
            pool_attr.update({"floating_ip_address": ""})
        ...
        with session.begin():
            subnet = session.query(models_v2.Subnet).filter(models_v2.Subnet.id == pool["subnet_id"]).first()
            if subnet:
                pool_attr.update({"cidr": subnet.cidr})
                pool_attr.update({"subnet_name": subnet.name})
            else:
                pool_attr.update({"cidr": ""})
                pool_attr.update({"subnet_name": ""})
        return pool_attr
    ...
    def _make_pool_dict(self, pool, fields=None):
        ...
        res.update(self._get_pool_attr(pool))
    ...
    def get_pool(self, context, id, fields=None):
        ...
        #alter by xm-20150609-for get vip_ip
        response = self._make_pool_dict(pool, fields)
        self._add_vip_ip_at_get_pool(pool, response)
        return response

    def _add_vip_ip_at_get_pool(self, pool, response):
        vip_address = ''
        vip=pool.get('vip', None)
        if vip:
            port=vip.get('port', None)
            if port:
                fixed_ips = port.get('fixed_ips',None)
                if fixed_ips:
                    vip_address = fixed_ips[0].get('ip_address', None)
        response.update({"vip_address": vip_address})

    def _get_subnet_ids_by_cidr(self, filters):
        #brk(host="10.10.0.90",port=50269)
        session = db.get_session()
        subnet_ids_from_filters = filters.get("subnet_id", [])
        cidr_from_filters = filters.get("cidr", [])
        subnet_ids = []
        for cidr in cidr_from_filters:
            with session.begin():
                query = session.query(models_v2.Subnet).filter(models_v2.Subnet.cidr == cidr)
                subnets = query.all()
                if subnets:
                    for subnet in subnets:
                        subnet_ids.append(subnet.id)
        if subnet_ids_from_filters:
            subnet_ids = list(set(subnet_ids_from_filters) & set(subnet_ids))
        return subnet_ids

    # def get_pools(self, context, filters=None, fields=None, sorts=None, limit=None, marker=None, page_reverse=None, offset=None):
    ...
    def get_pools(self, context, filters=None, fields=None, sorts=None, limit=None, marker=None, page_reverse=None, offset=None):
        #brk(host="10.10.0.90",port=50269)
        #add by xm-20150528-for find by 'cidr'
        if filters and 'cidr' in filters:
            subnet_ids = self._get_subnet_ids_by_cidr(filters)
            filters.update({"subnet_id": subnet_ids})

        return self._get_collection(context, Pool,
    ...
    ########################################################
    # Member DB access
    def _get_resource_name_by_pool_id(self, pool_id):
        session = db.get_session()
        with session.begin():
    ...
    def _make_member_dict(self, member, fields=None):
        res = {'id': member['id'],
               ...
               'resource_name': self._get_resource_name_by_pool_id(member['pool_id'])}
    ...
    #                                 filters=filters, fields=fields)
    def get_members(self, context, filters=None, fields=None, sorts=None, limit=None, marker=None, page_reverse=None, offset=None):
        #brk(host="10.10.12.21", port=49175)
        #add by xm-2015.7.14 支持按非members表中字段-resource_name的排序
        flag = False
        sort_dir = False
        if sorts:
            for sort in sorts:
                if sort[0] == u'resource_name':
                    flag = True
                    sort_dir = sort[1]
                    sorts.remove(sort)
                    break
        ...
        if 'resource_name' in filters:
            ...
            del filters['resource_name']
            filters.update({"pool_id": pool_ids})
        res = self._get_collection(context, Member,
                                    self._make_member_dict,
                                    filters=filters, fields=fields,
                                    sorts=sorts,
                                    limit=limit,
        ...
        #add by xm-2015.7.14 支持按非members表中字段-resource_name的排序
        if flag:
            res.sort(lambda x, y: cmp(x['resource_name'], y['resource_name']), reverse=not sort_dir)
        return res
        #end-2015.7.14
    ...
    def _make_health_monitor_dict(self, health_monitor, fields=None):
        res = {'id': health_monitor['id'],
               ...
               'user_id': health_monitor['user_id'],
               'create_time': health_monitor['create_time']}
    ...
    #                                 filters=filters, fields=fields)
    def get_health_monitors(self, context, filters=None, fields=None, sorts=None, limit=None, marker=None, page_reverse=None, offset=None):
        #add by xm-20150608-for filter by pool_id
        self._get_health_monitors_by_pool_id(context, filters)
        return self._get_collection(context, HealthMonitor,
                                    self._make_health_monitor_dict,
    ...
    #add by xm-20150608-for filter by pool_id---call by get_health_monitors(***)
    def _get_health_monitors_by_pool_id(self, context, filters):
        #add by xm-20150608-for find by 'pool_id'
        if filters and 'pool_id' in filters:
            #self._get_subnet_ids_by_cidr(filters)
            pool_id = filters.get("pool_id", [])
            monitor_ids_1 = filters.get('id', [])
            monitor_ids_2 = []
            with context.session.begin():
                    column = getattr(PoolMonitorAssociation, 'pool_id', None)
                    if column:
                        query = context.session.query(PoolMonitorAssociation).filter(column.in_(pool_id))
                        records = query.all()
                        if records:
                            for record in records:
                                monitor_ids_2.append(record.monitor_id)

            monitor_ids = list(set(monitor_ids_1) | set(monitor_ids_2))
            del filters["pool_id"]
            filters.update({"id": monitor_ids})

    ###add by xm-2015.8.28 -for bug6006-call by neutron/plugins/ml2/plugin.py/736: def delete_subnet
    def get_pools_by_subnet_id(self, subnet_id):
        pools = []
        session = db.get_session()
        query = session.query(Pool).filter_by(subnet_id=subnet_id)
        records = query.all()
        if records:
            for record in records:
                pools.append(record.id)
        return pools