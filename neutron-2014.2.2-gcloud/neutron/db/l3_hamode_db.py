...
L3_HA_OPTS = [
    ...
    cfg.BoolOpt('mulity_ha_network_one_tenant',
                default=True,
                help=_('Decide whether or not a tenant multiple ha networks')),
]
...
class L3HARouterNetworkForGcloud(model_base.BASEV2):
    """
    add by zyk  2016.3.3
    one  tenant has mulity networks
    """
    __tablename__ = 'ha_router_networks_for_gcloud'
    tenant_id = sa.Column(sa.String(255), nullable=False)
    #router_id = sa.Column(sa.String(255), nullable=False)
    network_id = sa.Column(sa.String(36),
                           sa.ForeignKey('networks.id', ondelete="CASCADE"),
                           nullable=False, primary_key=True)
    network = orm.relationship(models_v2.Network)

class RouterHaNetworkInfo(model_base.BASEV2):
    """
    add by zyk  2016.3.3
    """
    __tablename__ = 'router_ha_network_info'
    router_id = sa.Column(sa.String(255),  sa.ForeignKey('routers.id', ondelete="CASCADE"),
...
class L3_HA_NAT_db_mixin(l3_dvr_db.L3_NAT_with_dvr_db_mixin):
    ...
    def get_ha_network(self, context, tenant_id, router_id=None):
        if cfg.CONF.mulity_ha_network_one_tenant:
           return (context.session.query(RouterHaNetworkInfo).
                    filter(RouterHaNetworkInfo.router_id == router_id).
                    first())
        else:
    ...
    def get_ha_network_list(self, context, tenant_id):
        return (context.session.query(L3HARouterNetworkForGcloud).
                filter(L3HARouterNetworkForGcloud.tenant_id == tenant_id).
                all())
    ...
    def _create_ha_network_tenant_binding(self, context, tenant_id,
        ...
        with context.session.begin(subtransactions=True):
            if cfg.CONF.mulity_ha_network_one_tenant:
                ha_network = L3HARouterNetworkForGcloud(tenant_id=tenant_id,
                                           network_id=network_id)
            else:
    ...
    def get_number_of_agents_for_scheduling(self, context):
        ...
        ###alter by xm at 2015.9.24  add "active=1"
        #/usr/lib/python2.7/site-packages/neutron/db/l3_agentschedulers_db.py(354)get_l3_agents()
        num_agents = len(self.get_l3_agents(context, active=1))
        ###end
        max_agents = cfg.CONF.max_l3_agents_per_router
        #LOG.debug(_("111111 mim:%s max:%s  now:%s"), min_agents,max_agents,num_agents)
        if max_agents:
    ...
    def _create_ha_port_binding(self, context, port_id, router_id):
        ...
        return portbinding
    #RouterHaNetworkInfo
    def _create_router_ha_network_info(self, context, router_id, network_id):
            with context.session.begin(subtransactions=True):
                info = RouterHaNetworkInfo(router_id=router_id, network_id=network_id)
    ...
    def delete_router(self, context, id):
        ...
        if router_db.extra_attributes.ha:
            ha_network = self.get_ha_network(context,
                                             router_db.tenant_id, router_id=id)
            if ha_network:
                self._delete_vr_id_allocation(
                    context, ha_network, router_db.extra_attributes.ha_vr_id)
                self._delete_ha_interfaces(context, router_db.id)
