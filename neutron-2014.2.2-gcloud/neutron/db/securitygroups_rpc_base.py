...
class SecurityGroupServerRpcMixin(sg_db.SecurityGroupDbMixin):
    ...
    def security_group_info_for_ports(self, context, ports):
        ...
        #LOG.info(_('123123: %s'), rules_in_db.__dict__)
        ...
        for (port_id, rule_in_db) in rules_in_db:
            ...
            for key in ('protocol', 'port_range_min', 'port_range_max',
                ...
                ###add by xm at 2015.9.1 for icmp(type=0,code=0)---begin---
                if key in ('port_range_min', 'port_range_max') \
                        and rule_in_db.get(key) == 0L \
                        and rule_in_db.get('protocol') =='icmp':
                    rule_dict[key] = rule_in_db[key]
                ###end
        ...
        #add by xm-2015.8.21-for bug 5713-par1---start
        sgs_bindings_in_db = self._get_sg_for_ports_2(context, ports)
        LOG.info(_('123123 sgs_bindings_in_db: %s'), sgs_bindings_in_db)
        for (port_id, sg_binding_in_db) in sgs_bindings_in_db:
            security_group_id = sg_binding_in_db.get('security_group_id')
            if security_group_id not in sg_info['security_groups']:
                sg_info['security_groups'][security_group_id] = []
        #add by xm-2015.8.21-for bug 5713-par1---end
        ...
        #add by xm-2015.8.21-for debug
        xx = self._get_security_group_member_ips(context, sg_info)
        LOG.debug(_('123123 sg_info: %s'), xx)
        return xx
    ...
    #add by xm-2015.8.21-for bug 5713-par2---start
    def _get_sg_for_ports_2(self, context, ports):
        if not ports:
            return []
        sg_binding_port = sg_db.SecurityGroupPortBinding.port_id
        query = context.session.query(sg_binding_port, sg_db.SecurityGroupPortBinding)
        query = query.filter(sg_binding_port.in_(ports.keys()))
        return query.all()
    #add by xm-2015.8.21-for bug 5713-par2---end
