...
class NeutronDbPluginV2(neutron_plugin_base_v2.NeutronPluginBaseV2,
    ...
    def register_dict_extend_funcs(cls, resource, funcs):
        ...
        cls._dict_extend_functions[resource] = cur_funcs    #为各个资源注册扩展方法：如db/l3_gwmode_db.py line 38
        # db_base_plugin_v2.NeutronDbPluginV2.register_dict_extend_funcs(l3.ROUTERS, ['_extend_router_dict_gw_mode'])
    ...
    def get_infos_of_gateways(self, context, ports):
        LOG.debug("get_infos_of_gateways in call()")
        infos = []
        query = context.session.query(models_v2.Port)
        #query.join(models_v2.IPAllocation)
        records = query.filter(models_v2.Port.id.in_(ports)).all()
        LOG.debug("records %r" %(records))
        if records:
            for record in records:
                ip_info_dict={"port_id": record.id, "router_id": record.device_id}
                if record.fixed_ips and len(record.fixed_ips)>0:
                    ip_info_dict.update( {"ip_address": record.fixed_ips[0].ip_address,"subnet_id":record.fixed_ips[0].subnet_id})
                else:
                    ip_info_dict.update( {"ip_address": None,"subnet_id":None})
                infos.append(ip_info_dict)
        LOG.debug("port infos %r" %(infos))
        return infos
        ...
        for fixed in fixed_ips:
            ...
            if 'subnet_id' not in fixed:
                if 'ip_address' not in fixed:
                    ...
                    raise n_exc.InvalidInput(error_message=msg,gcloud_code='neutron_server_port_000005')
                ...
                if not found:
                    ...
                    raise n_exc.InvalidInput(error_message=msg,gcloud_code='neutron_server_port_000006')
            else:
                ...
                if subnet['network_id'] != network_id:
                    ...
                    raise n_exc.InvalidInput(error_message=msg,gcloud_code='neutron_server_port_000007')
                subnet_id = subnet['id']
            ...
            if 'ip_address' in fixed:
                ...
                if (not found and
                    ...
                    raise n_exc.InvalidInput(error_message=msg,gcloud_code='neutron_server_port_000008')
    ...
    def _allocate_ips_for_port(self, context, port):
        ...
        ###add by xm for tz_project gcloud_nat 2015.12.12
        LOG.debug(_("111111 create 4 fixed_configured=%s  "),fixed_configured)
        if port.get("tz_project_flag",None) =="flag":
            fixed_configured = False
        LOG.debug(_("111111 create 4 fixed_configured=%s  "),fixed_configured)
        ###end 2012.12.12
        if fixed_configured:
            LOG.debug(_("111111 create 77 fixed_ips=%s  "), p['fixed_ips'])
    ...
    def _validate_subnet_cidr(self, context, network, new_subnet_cidr):
        ...
        for cidr in new_subnet_ipset.iter_cidrs():
            if cidr.prefixlen == 0:
                ...
                raise n_exc.InvalidInput(error_message=err_msg,gcloud_code='neutron_server_subnet_000009')
            ...
            if (netaddr.IPSet([subnet.cidr]) & new_subnet_ipset):
                ...
                LOG.info(_("Validation for CIDR: %(new_cidr)s failed - "
                         ...
                         {'new_cidr': new_subnet_cidr,
                          'subnet_id': subnet.id,
                          'cidr': subnet.cidr})
                raise n_exc.InvalidInput(error_message=err_msg,gcloud_code="neutron_server_subnet_000008")
    ...
    def _make_network_dict(self, network, fields=None,
        ...
        res = {'id': network['id'],
               ...
               # add by xm-2015.5.15-subnets_num,user_name
               'subnets_num': len(network['subnets']),
               #'user_name': 'xmtest'
               "user_id" : network['user_id'],
               "create_time": network['create_time']
               }
    ...
    def _get_routers_by_subnet_id(self, subnet_id):
        #============method1和method2原本在get_subnets函数中，后调整为在_make_subnet_dict函数实现================================
        # method 1:
        # if subnets:
        #     for subnet in subnets:
        #         #filters_ports={"device_owner": ["network:router_interface"], "fixed_ips":{"subnet_id": ["ee9ccdf3-cc58-47f0-bc22-16c7cb728325"]}}
        #         filters_ports={"device_owner": ["network:router_interface", "network:router_gateway"], "fixed_ips": {"subnet_id": [subnet["id"]]}}
        #         ports = self.get_ports(context, filters_ports)
        #         routers = []
        #         if ports:
        #             for port in ports:
        ...
        #                         device_id=router_id)
        #                 router ={"router_id": router["id"], "name": router["name"]}
        #                 routers.append(router)
        #         dict_routers = {"routers": routers}
        #         subnet.update(dict_routers)
        #============================================
        #============================================
        # #method 2:
        # if subnets:
        #      for subnet in subnets:
        #          routers = []
        #          query = context.session.query(l3_db.RouterPort).filter_by(subnet_id=subnet["id"])
        #          records = query.all()
        #          if records:
        #              for record in records:
        #                 query_router = context.session.query(l3_db.Router).filter_by(id=record.router_id).first()
        ...
        if l3plugin:
            routers = l3plugin._get_routers_by_subnet_id(subnet_id)
        else:
    ...
    def _make_subnet_dict(self, subnet, fields=None):
        res = {'id': subnet['id'],
               ...
               "user_id": subnet['user_id'],
               "create_time": subnet['create_time'],
               "type":subnet['type'],
               "routers": self._get_routers_by_subnet_id(subnet_id=subnet['id'])
               }
        # Call auxiliary extend functions, if any
        self._apply_dict_extend_functions(attributes.SUBNETS, res, subnet)
        return self._fields(res, fields)

    def _get_subnet_by_subnet_id(self,subnet_id):
        """
        make extend port attribute ,add subnet
        """
        session=db.get_session()
        with session.begin():
               subnet=session.query(models_v2.Subnet).filter(models_v2.Subnet.id == subnet_id ).first()
               if subnet:
                   return subnet
        return None

    def _get_subnet_ids_by_subnet_name(self,context,subnet_names):
        subnets=context.session.query(models_v2.Subnet).filter(models_v2.Subnet.name.in_(subnet_names)).all()
        if subnets:
                return [ subnet["id"] for subnet in subnets]
        return [""]
    def _get_network_name_by_network_id(self,network_id):
        session = db.get_session()
        with session.begin():
          network = session.query(models_v2.Network).filter(models_v2.Network.id  ==  network_id).first()
          if network:
                return  network["name"]
        return ""
    ...
    def _make_port_dict(self, port, fields=None,
        ...
        res = {"id": port["id"],
               ...
               "network_name":self._get_network_name_by_network_id( port["network_id"]),
               'tenant_id': port['tenant_id'],
               "mac_address": port["mac_address"],
               "admin_state_up": port["admin_state_up"],
               "status": port["status"],
               "fixed_ips": [{'subnet_id': ip["subnet_id"],
                              # 'subnet_name':self._get_subnet_name_by_subnet_id(ip["subnet_id"]),
                              'ip_address': ip["ip_address"]}
                             for ip in port["fixed_ips"]],
               "device_id": port["device_id"],
               "device_owner": port["device_owner"],
               "create_time":port["create_time"],
               "user_id":port["user_id"],
               "used_status":port["used_status"]}
        subnets=res.get("fixed_ips")
        #update subnet info,include adding subnet_name,and cidr
        for subnet in subnets:
                subnet_db=self._get_subnet_by_subnet_id(ip["subnet_id"])
                if subnet_db:
                     subnet["subnet_name"]=subnet_db["name"]
                     subnet["cidr"]=subnet_db["cidr"]
    ...
    def get_network(self, context, id, fields=None):
        network = self._get_network(context, id)
        #add by xm-2015.5.15-user_name,can not do it so,chose other way!!
        #network.update({"user_name": context.user_name})
        return self._make_network_dict(network, fields)

    def get_networks(self, context, filters=None, fields=None,
                     sorts=None, limit=None, marker=None,
                     page_reverse=False, offset=None):
        marker_obj = self._get_marker_obj(context, 'network', limit, marker) #get marker record
        #add 2015-7-14 by luoyibing

        self._update_filters_by_provider_network_type(context, filters)
        #end 2015-7-14 by luoyibing

        # ### add by xm and luoyb at 2015.9.22
        # if filters.get("user_id", None):
        #     user_id = filters.pop("user_id")
        #     filters["column_names"] = ["shared", "user_id"]
        #     filters["column_values"] = [[True], user_id]
        # ###end

        res= self._get_collection(context, models_v2.Network,
    ...
    def _update_filters_by_provider_network_type(self,context, filters):
        vals = filters and filters.get(provider.NETWORK_TYPE, [])
        id_vals = filters and filters.get("id", [])
        if vals:
            with context.session.begin(subtransactions=True):
                query = (context.session.query(models.NetworkSegment).
                         filter_by(network_type=vals[0]))
                records = query.all()
                netowrk_ids = [record.network_id for record in records]
                if id_vals:
                    netowrk_ids = list(set(id_vals) & set(netowrk_ids))
                filters.update({"id": netowrk_ids})

    def get_networks_count(self, context, filters=None):
        #add by xm-20150529
        self._update_filters_by_provider_network_type(context, filters)

        # ### add by xm luoyb at 2015.9.22
        # if filters.get("user_id", None):
        #     user_id = filters.pop("user_id")
        #     filters["column_names"] = ["shared", "user_id"]
        #     filters["column_values"] = [[True], user_id]
    ...
    def _validate_subnet(self, context, s, cur_subnet=None):
        ...
        if attributes.is_attr_set(s.get('gateway_ip')):
            ...
            if (cfg.CONF.force_gateway_on_subnet and
                ...
                raise n_exc.InvalidInput(error_message=error_message,gcloud_code="neutron_server_subnet_000007")
    ...
    def create_port(self, context, port):
        ...
        if p.get('device_owner') == constants.DEVICE_OWNER_ROUTER_INTF:#if this is route interface
        ...
        with context.session.begin(subtransactions=True):
            ...
            db_port = models_v2.Port(tenant_id=tenant_id,
                                     ...
                                     user_id=context.user,used_status=used_status)
            context.session.add(db_port)

            ### changed by xm for tengzheng project : do not allocate external ip
            # to the netowrk:router_gateway port at 2015.12.7
            #if device_owner is network:router_gateway do not allocate external ip
            #LOG.debug(_("111111 db_base_plugin_v2 7788 2 allow_gcloud_nat=%d"), cfg.CONF.allow_gcloud_nat)
            if p.get('device_owner') == constants.DEVICE_OWNER_ROUTER_GW and cfg.CONF.allow_gcloud_nat:
                #LOG.debug(_("111111 db_base_plugin_v2 7788 3 allow_gcloud_nat"))
                pass
            else:
            ...
            ### end by xm at 2015.12.7 ###

        #LOG.debug(_("111111 db_base_plugin_v2 7788 4  allow_gcloud_nat=%d"), cfg.CONF.allow_gcloud_nat)
        return self._make_port_dict(db_port, process_extensions=False)

    ### add by xm for tengzheng project at 2015.12.7
    def tz_allocate_ips_for_gateway_port(self, context, gw_port, flag=True):
        '''
        when user in tz project start to create a port nat,
        first to allocate external ip for the network:router_gateway port
        '''
        # Update the IP's for the port
        if flag:
            port ={"port": gw_port, "tz_project_flag": "flag"}
        else:
            port = {"port": gw_port}
        ips = self._allocate_ips_for_port(context, port)
        ...
        if ips:
            for ip in ips:
                ip_address = ip['ip_address']
                subnet_id = ip['subnet_id']
                ip.update({"network_id": network_id})
                NeutronDbPluginV2._store_ip_allocation(
                    context, ip_address, network_id, subnet_id, port_id)
        return ips

    def tz_delete_allocated_ips_of_gateway_port(self,context,ipallocation):

        self._delete_ip_allocation(context, ipallocation['network_id'],
                                    ipallocation['subnet_id'],
                                    ipallocation['ip_address'])


    def tz_get_port(self,context, id):
        port = self._get_port(context, id)
        subnet = self._get_subnet(context, port.fixed_ips[0]["subnet_id"])
        return port, subnet
    ...
    def _get_ports_query(self, context, filters=None, sorts=None, limit=None,
                         marker_obj=None, page_reverse=False,offset=None):
        Port = models_v2.Port
        ...
        #add search subnet_name 20150520
        subnet_names=fixed_ips.get('subnet_name',None)
        if subnet_names is not None:
            subnet_ids=self._get_subnet_ids_by_subnet_name(context,subnet_names)
