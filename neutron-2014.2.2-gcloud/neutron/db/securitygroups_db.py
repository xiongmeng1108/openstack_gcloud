...
class SecurityGroup(model_base.BASEV2, models_v2.HasId,models_v2.HasCreateTime, models_v2.HasTenant):
...
class SecurityGroupDbMixin(ext_sg.SecurityGroupPluginBase):
    ...
    def create_security_group(self, context, security_group, default_sg=False):
        ...
        with context.session.begin(subtransactions=True):
            security_group_db = SecurityGroup(id=s.get('id') or (
                                              ...
                                              tenant_id=tenant_id,
                                              name=s['name'],user_id=context.user)
    ...
    def get_security_groups(self, context, filters=None, fields=None,
                            ...
                            marker=None, page_reverse=False, offset=None,default_sg=False):
        ...
        ### delete by xm and zyk at 2015.9.24 for avoid one pt_user one default
    ...
    def _make_security_group_dict(self, security_group, fields=None):
        res = {'id': security_group['id'],
               ...
               'create_time': security_group['create_time'],
               'user_id': security_group['user_id'],
               'status':self._get_security_groups_status( security_group['id'])
               }
    ...
    def _get_security_groups_on_port(self, context, port):
        ...
        if port_sg_missing:
            sg_ids=''
            for sg_id in port_sg_missing:
                sg_ids+=sg_id+","
            raise ext_sg.SecurityGroupNotFound(id=sg_ids)
    ...
    def _ensure_default_security_group_on_port(self, context, port):
        ...
        if (port['port'].get('device_owner') and
            ...
            return   #if device_owner is network begin return
        ...
        if attr.is_attr_set(port['port'].get(ext_sg.SECURITYGROUPS)):
            ...
            #sgids = [default_sg]
            # add follow
            #raise ext_sg.SecurityGroupNotSpecified
            return
            ### end
    ...
    def _check_update_has_security_groups(self, port):
        ...
        if (ext_sg.SECURITYGROUPS in port['port'] and
            ...
            return True
        return False
    #add by luoyibing for test
    def get_securitygroupportbing(self, context, filters=None, fields=None,
                                 sorts=None, limit=None, marker=None,
                                 page_reverse=False):
        db_base_plugin_v2.NeutronDbPluginV2.get_ports(self, context, filters=None, fields=None,
                  sorts=None, limit=None, marker=None,
                  page_reverse=False)

    def _get_security_groups_status(self,id):
        """
        get security groups status
        return:Boolean ,if security group has not port ,return False;else return True
        """
        session=db.get_session()
        with session.begin():
               count=session.query(SecurityGroupPortBinding).filter_by( security_group_id = id ).count()
               if count:
                   return True
        return False

    """
    def _get_security_groups_ids_by_status(self,context,status):
        if status == True:
            securityGroupPortBindings=self._get_collection(context, SecurityGroupPortBinding)
            if securityGroupPortBindings:
                return [securityGroupPortBinding["security_group_id"] for securityGroupPortBinding in securityGroupPortBindings ]
            return None
        else:
            #if status == False:
            #get securitygroup  status
    """
