...
from neutron.plugins.ml2 import gcloud_identity_config  #add for support report info to gcloud_identity
...
from neutron.common import exceptions as n_exc #add 20150720 throws exception
from neutron.extensions import securitygroup as ext_sg
from neutron.openstack.common import loopingcall
 #add for support report info to gcloud_identity
from neutron.plugins.ml2 import gcloud_identity_client
...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def __init__(self):
        ...
        self.extension_manager = managers.ExtensionManager()  #eg:neutron.ml2.extension_drivers
        self.mechanism_manager = managers.MechanismManager()  #eg: openvswitch,linuxbridge,....
        ...
        self._setup_loop_reporting()

        LOG.info(_("Modular L2 Plugin initialization complete"))


    #2015-9-17 add by luoyibing for  loop thread for report floating ip infos
    def  _setup_loop_reporting(self):
        LOG.info(_("_setup_loop_reporting begin"))
        try:
          self.report_floatingIp_interval = cfg.CONF.gc_identity_report["report_floatingIp_interval"]     #.report_interval
          self.report_user_name= cfg.CONF.gc_identity_report["admin_user"]
          self.report_user_pass= cfg.CONF.gc_identity_report["admin_password"]
          self.report_tenant_name=cfg.CONF.gc_identity_report["admin_tenant_name"]
          self.region_id = cfg.CONF.gc_identity_report["region_id"]
        except Exception,e:
            LOG.error("%s"%e)
            LOG.error("please configure items [ report_floatingIp_interval ,admin_user,admin_password,admin_tenant_name ] in gc_identity_report of neutron.cfg")
            return
        if self.report_floatingIp_interval>0 and self.region_id is not  None:
            heartbeat = loopingcall.FixedIntervalLoopingCall(
                self._report_floatingIp)
            heartbeat.start(interval=self.report_floatingIp_interval)

        else:
            LOG.info("not report_floatingIp ,because report_floatingIp_interval is not bigger than zero and region_id must be configured ")
        LOG.info(_("_setup_loop_reporting end"))

    def _report_floatingIp(self):
        LOG.info(_("_report_floatingIp"))
        try:

            l3plugin = manager.NeutronManager.get_service_plugins().get(
                    service_constants.L3_ROUTER_NAT)
            if l3plugin:
                    #get user floating ip info
                    #LOG.info(_("_report_floatingIp1"))
                    #(self,user_name=None,user_pass=None,tenantName=None,auth_url=None,region_id=None):
                    client=gcloud_identity_client.Client(user_name=self.report_user_name, #cfg.CONF.admin_tenant_name ,
                                                        user_pass=self.report_user_pass, #cfg.CONF.admin_password,
                                                        tenantName=self.report_tenant_name,
                                                        auth_url=cfg.CONF.keystone_authtoken["auth_uri"],
                                                        region_id=self.region_id)
                    #LOG.info(_("_report_floatingIp2"))
                    floatingipInfos=l3plugin.get_userid_floatingips_count(context=client.ctx)
                    #LOG.info(_("_report_floatingIp3"))
                    for floatingipInfo in floatingipInfos:
                            LOG.info( _("floatingip user_is %(user_id)s ,count %(count)d",
                                      {'user_id':floatingipInfo[0], 'count': int(floatingipInfo[1])}))
                            #call gcloud_identify api
                            client.report_floatingip_count(userId=floatingipInfo[0],count=floatingipInfo[1])
        except Exception, e:
             LOG.debug("Exception is  %s" %e)
        LOG.info(_("_report_floatingIp_end"))
    def _setup_rpc(self):
        self.notifier = rpc.AgentNotifierApi(topics.AGENT)
        self.agent_notifiers[const.AGENT_TYPE_DHCP] = (
            dhcp_rpc_agent_api.DhcpAgentNotifyAPI()
        )
#rpc service  linster,called by service.py RpcWorker: start fucntion
    ...
    def _filter_nets_provider(self, context, nets, filters):
        ...
        with context.session.begin(subtransactions=True):
            ...
            return [n for n in nets if n["id"] in netowrk_ids]
    ...
    def create_network(self, context, network):
        ...
        with session.begin(subtransactions=True):
            ### delete by xm and zyk at 2015.9.24 for gcloud7 avoid one pt_user one default
            #self._ensure_default_security_group(context, tenant_id)#Create a default security group if one doesn't exist
            ###end
    ...
    def get_networks(self, context, filters=None, fields=None,
        ...
        if sorts:
            for sort in sorts:
                if sort[0] == provider.NETWORK_TYPE:
                    flag = True
                    sort_dir = sort[1]
                    sorts.remove(sort)
                    break
        ...
        with session.begin(subtransactions=True):
            nets = super(Ml2Plugin,
                         self).get_networks(context, filters, None, sorts,
                                            limit, marker, page_reverse, offset=offset)
            for net in nets:
        ...
        res= [self._fields(net, fields) for net in nets]
        if flag:
            res.sort(lambda x, y: cmp(x[provider.NETWORK_TYPE], y[provider.NETWORK_TYPE]), reverse=not sort_dir)
        #end-2015.7.14
        return res
    ...
    def delete_subnet(self, context, id):
        ...
        while True:
            ...
            with contextlib.nested(lockutils.lock('db-access'),
                ...
                ###add by xm-2015.8.28 -for bug6006: decide the subnet which is deleting has beed associated to pool
                self._subnet_has_been_associated_to_pool(subnet_id=id)
                ###end
    ...
    ###add by xm-2015.8.28 -for bug6006
    def _subnet_has_been_associated_to_pool(self, subnet_id):
        lbplugin = manager.NeutronManager.get_service_plugins().get(service_constants.LOADBALANCER)
        if lbplugin:
            pools = lbplugin.get_pools_by_subnet_id(subnet_id=subnet_id)
            if pools:
                LOG.error(_("subnet %s is in used, which has been associated to pool, can't delete it"), subnet_id)
                raise exc.SubnetInUse(subnet_id=subnet_id)
        else:
            LOG.info(_('No plugin for Lbaas registered'))

    def _get_port_ids_by_sgids(self,context,filters):
        """
        get port ids by securgroup_group_id
        return: port ids
        """
        security_group_ids = filters.pop('security_group_id', None)
        if security_group_ids:
                security_group_ids = {"security_group_id": security_group_ids}
                security_groups_bindings = self._get_port_security_group_bindings(context,security_group_ids )
                if security_groups_bindings:
                        port_ids=[ security_groups_binding["port_id"] for security_groups_binding in security_groups_bindings ]
                        #filters["id"]=
                        return port_ids
                else:
                    return None
    ...
    def get_ports(self, context, filters=None, fields=None,
        ...
        if filters:
               #get filters security_gorups
               if filters.get('security_group_id') is not None:
                     port_ids=self._get_port_ids_by_sgids(context,filters)
                     if port_ids:
                        filters["id"]=port_ids
                     else:
                         return []

        return super(Ml2Plugin, self).get_ports(context, filters=filters, fields=fields,
               ...
               if filters.get('security_group_id') is not None:
                     ...
                     if port_ids:
                        filters["id"]=port_ids
                         ...
                         return 0
        return super(Ml2Plugin, self).get_ports_count(context,filters)

    def create_port(self, context, port):
        attrs = port['port'] #value??
    ...
    def update_port(self, context, id, port):
        ...
        ###add by xm at 2015.9.6 for bug 6306---begin---###
        self._notify_agents_sgs_changed_by_xm(context, original_port, updated_port)
        #self.notify_security_groups_member_updated(context, original_port)
        ###add by xm at 2015.9.6 for bug 6306---end---###


        return bound_port._port

    ###add by xm at 2015.9.6 for bug 6306###
    def _notify_agents_sgs_changed_by_xm(self, context, original_port, updated_port):
        sgs_xm_orig = original_port.get(ext_sg.SECURITYGROUPS)
        sgs_xm_now = updated_port.get(ext_sg.SECURITYGROUPS)
        sgs_xm = list(set(sgs_xm_orig) | set(sgs_xm_now))
        self.notifier.security_groups_member_updated(context, sgs_xm)
    ...
    def delete_port(self, context, id, l3_port_check=True):
        ...
        if l3plugin and l3_port_check:
            l3plugin.prevent_l3_port_deletion(context, id)  # Checks to make sure a port is allowed to be deleted
        ...
        with contextlib.nested(lockutils.lock('db-access'),
                               session.begin(subtransactions=True)):
            port_db, binding = db.get_locked_port_and_binding(session, id)#"""Get port and port binding records for update within transaction."""
            if not port_db:
            ...
            if device_owner == const.DEVICE_OWNER_DVR_INTERFACE:
                ...
                if device_owner == "compute:node":
                       if port['device_id'] != None and len(port['device_id']) != 0: #add by zyk 2015.11.30  and len(port['device_id']) != 0
                              raise n_exc.PortInUse(net_id=port['network_id'],  #add by luoyibing 20150720
                                      port_id=port['id'],
