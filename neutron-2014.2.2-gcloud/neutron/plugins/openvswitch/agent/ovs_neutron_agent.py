...
class OVSQosAgent(object):
    ...
    def treat_qos_added(self, device, qos_rule):
        ...
        if not vif_port:
            LOG.debug("qosdebug: spch_device_not_in")
            return
        #pdb.set_trace()
        self.int_br.set_interface_qos(vif_port.port_name, qos_rule['ingress'], qos_rule['outgress'])

    #devices对应的是数据库端口id，不是br-int上的接口id
    def clear_qos_rules(self, devices):
        for device in devices:
            vif_port = self.int_br.get_vif_port_by_id(device)
            if not vif_port:
                LOG.debug(_("qosdebug:external_ids:iface-id=%s not exist in br-int,can not clear qos_rules" % device))
                continue
            self.int_br.clear_interface_qos(vif_port.port_name)

    def apply_qos_rules(self, devices):
        #pdb.set_trace()  #by xm
        if not devices:
            LOG.debug("qosdebug: spch_devices_is_NULL")
        for device in devices:
            try:
                qos_rule = self.plugin_rpc.get_qos_details(self.context, device, self.agent_id)
            except Exception:
                LOG.debug(_("qosdebug: spch_Unable to get port qos details for device_id = %s" % device))
                qos_rule = {u'ingress': None, u'outgress': None}
                #continue
            LOG.debug(_("qosdebug: device_id = %s  qos_rule=%s " % (device, qos_rule)))
            if qos_rule is None or qos_rule == {}:
                qos_rule = {u'ingress': None, u'outgress': None}
...
class OVSNeutronAgent(n_rpc.RpcCallback,
    ...
    def __init__(self, integ_br, tun_br, local_ip,
        ...
        self.setup_rpc()  #创建定时任务向neutron-server报告ovs agent状态，另外创建了多个主题的消费者
    ...
    def setup_rpc(self):
        self.agent_id = 'ovs-agent-%s' % cfg.CONF.host  #如self.agent_id="ovs-agent-controller"
        self.topic = topics.AGENT  #self.topic="q-agent-notifier"
        self.plugin_rpc = OVSPluginApi(topics.PLUGIN)
        self.state_rpc = agent_rpc.PluginReportStateAPI(
            topics.PLUGIN)  #调用了oslo.messageing-1.4.1项目的messaging.Target；neutron/common/rpc.py的序列化
        #以上两个RPCapi对象，用于向neutron-server发送消息，如state_rpc调用report_state向ML2插件报告agent状态
        # RPC network init
        ...
        consumers = [[topics.PORT, topics.UPDATE],
                     ...
                     [topics.DVR, topics.UPDATE]]  #定义消费者主题
        if self.l2_pop:
        ...
        self.connection = agent_rpc.create_consumers(self.endpoints,
                                                     self.topic,
                                                     consumers,
                                                     start_listening=False)  #为每个主题创建消费者
        report_interval = cfg.CONF.AGENT.report_interval
        if report_interval:
            heartbeat = loopingcall.FixedIntervalLoopingCall(
                self._report_state)
            heartbeat.start(interval=report_interval)  #启动定时线程向neutron-server上报ovs-agent状态
    ...
    def qos_update(self, context, **kwargs):
        ...
        # they are not used since there is no guarantee the notifications
        # are processed in the same order as the relevant API requests
        self.updated_ports.add(qos['port_id'])
        LOG.debug(_("qos_update message processed for port %s"), qos['port_id'])
        ...
        for lvm, agent_ports in self.get_agent_ports(fdb_entries,
            ...
            if len(agent_ports):
                if not self.enable_distributed_routing:
                    with self.tun_br.deferred() as deferred_br:  #如果没有配置分布式路由，则将br-tun网桥变成“延期的”（fdb表流规则不及时更新，等有使用才更新？）
                        self.fdb_add_tun(context, deferred_br, lvm,
                                         agent_ports, self.tun_br_ofports)  #创建gre|vxlan隧道   ovs-vsctl show查看
                else:
    ...
    def add_fdb_flow(self, br, port_info, remote_ip, lvm, ofport):
        if port_info == q_const.FLOODING_ENTRY:
            lvm.tun_ofports.add(ofport)  #如：lvm.tun_ofports=set(['2'])  ofport=2即br-tun上的Port "gre-0a0a0056"
            ofports = ','.join(lvm.tun_ofports)  #ofports='2'
            br.mod_flow(table=constants.FLOOD_TO_TUN,
                        dl_vlan=lvm.vlan,
                        actions="strip_vlan,set_tunnel:%s,output:%s" %
                                (lvm.segmentation_id, ofports))  #
        else:  #如：port_info=[u'fa:16:3e:4f:04:59', u'169.254.192.9']
            self.setup_entry_for_arp_reply(br, 'add', lvm.vlan, port_info[0],
                                           port_info[
                                               1])  #/etc/neutron/plugins/openvswitch/ovs_neutron_plugin.ini:# arp_responder = False，为True才操作
    ...
    def setup_entry_for_arp_reply(self, br, action, local_vid, mac_address,
        ...
        tunnel bridge.
        '''
        if not self.arp_responder_enabled:  #/etc/neutron/plugins/openvswitch/ovs_neutron_plugin.ini:# arp_responder = False，为True才操作，默认为false
            return
    ...
    def port_bound(self, port, net_uuid,
        ...
        if net_uuid not in self.local_vlan_map or ovs_restarted:
            self.provision_local_vlan(net_uuid, network_type,
                                      physical_network, segmentation_id)  #为网络分配本地VLAN
    ...
    def setup_integration_br(self):
        ...
        self.int_br.create()  #创建br-int虚拟交换机：ovs-vsctl -- --may-exist add-br br-int
        self.int_br.set_secure_mode()  #ovs-vsctl -- set-fail-mode br-int secure

        self.int_br.delete_port(cfg.CONF.OVS.int_peer_patch_port)  #ovs-vsctl -- --if-exists del-port br-int patch-tun
        self.int_br.remove_all_flows()  #清除br-int上所有规则：ovs-ofctl del-flows br-int
    ...
    def setup_tunnel_br(self, tun_br_name=None):
        ...
        self.tun_br.set_secure_mode() #add by luoyibing 2016/1/14
        ...
        self.tun_br.add_flow(priority=1,
                             ...
                             actions="resubmit(,%s)" %
                                     constants.PATCH_LV_TO_TUN)  #cookie=0x0, duration=696.843s, table=0, n_packets=348, n_bytes=18792, idle_age=0, priority=1,in_port=1 actions=resubmit(,2)
        self.tun_br.add_flow(priority=0,
                             actions="drop")  #cookie=0x0, duration=582.375s, table=0, n_packets=0, n_bytes=0, idle_age=582, priority=0 actions=drop
        ...
        self.tun_br.add_flow(table=constants.PATCH_LV_TO_TUN,
                             ...
                             actions="resubmit(,%s)" % constants.UCAST_TO_TUN)  #cookie=0x0, duration=288.218s, table=2, n_packets=0, n_bytes=0, idle_age=288, priority=0,dl_dst=00:00:00:00:00:00/01:00:00:00:00:00 actions=resubmit(,20)
        # Broadcasts/multicasts go to table FLOOD_TO_TUN that handles flooding
        self.tun_br.add_flow(table=constants.PATCH_LV_TO_TUN,
                             priority=0,
                             dl_dst="01:00:00:00:00:00/01:00:00:00:00:00",
                             actions="resubmit(,%s)" % constants.FLOOD_TO_TUN)  # cookie=0x0, duration=170.590s, table=2, n_packets=85, n_bytes=4590, idle_age=0, priority=0,dl_dst=01:00:00:00:00:00/01:00:00:00:00:00 actions=resubmit(,22)
        ...
        for tunnel_type in constants.TUNNEL_NETWORK_TYPES:
            self.tun_br.add_flow(table=constants.TUN_TABLE[tunnel_type],
                                 priority=0,
                                 #for vxlan: cookie=0x0, duration=19.422s, table=4, n_packets=0, n_bytes=0, idle_age=19, priority=0 actions=drop
                                 actions="drop")  #for gre: cookie=0x0, duration=7.058s, table=3, n_packets=0, n_bytes=0, idle_age=7, priority=0 actions=drop
        ...
        self.tun_br.add_flow(table=constants.LEARN_FROM_TUN,
                             ...
                             actions="learn(%s),output:%s" %
                                     (learned_flow,
                                      self.patch_int_ofport))  #cookie=0x0, duration=5.892s, table=10, n_packets=0, n_bytes=0, idle_age=5, priority=1 actions=learn(table=20,hard_timeout=300,priority=1,NXM_OF_VLAN_TCI[0..11],NXM_OF_ETH_DST[]=NXM_OF_ETH_SRC[],load:0->NXM_OF_VLAN_TCI[],load:NXM_NX_TUN_ID[]->NXM_NX_TUN_ID[],output:NXM_OF_IN_PORT[]),output:1   #1对应br-tun上的patch-int口
        ...
        self.tun_br.add_flow(table=constants.UCAST_TO_TUN,
                             priority=0,
                             actions="resubmit(,%s)" %
                                     constants.FLOOD_TO_TUN)  #cookie=0x0, duration=8.420s, table=20, n_packets=0, n_bytes=0, idle_age=8, priority=0 actions=resubmit(,22)
        if self.arp_responder_enabled:
        ...
        self.tun_br.add_flow(table=constants.FLOOD_TO_TUN,
                             priority=0,
                             actions="drop")  #cookie=0x0, duration=6.822s, table=22, n_packets=3, n_bytes=162, idle_age=1, priority=0 actions=drop
    ...
    def scan_ports(self, registered_ports, updated_ports=None):
        cur_ports = self.int_br.get_vif_port_set()  #此处获取本地br-int的端口：set([u'0e10bc14-99ef-4eae-bb8e-d1b87e99a7da', u'8f9a5cdd-3deb-45d8-a68e-e71450c58081', u'6f3f6216-ef75-4e4f-866d-4100d2e5db22', u'9bf56d57-97a3-4de7-be67-5e10e3772db3', u'77bec540-b3e6-47a4-8dad-4cf802355acd', u'ee622161-cff8-48a8-9f00-3c49c1620cc2'])
    ...
    def check_changed_vlans(self, registered_ports):
        ...
        vlan tag loss.
        """
        port_tags = self.int_br.get_port_tag_dict()  #br-int上的端口和本地vlan：{u'qr-77bec540-b3': 2, u'qvoee622161-cf': 2, u'tap9bf56d57-97': 1, u'qr-8f9a5cdd-3d': 1, u'patch-tun': [], u'tap6f3f6216-ef': 2, u'int-br-ex': [], u'ha-0e10bc14-99': 3}
        changed_ports = set()
    ...
    def _setup_tunnel_port(self, br, port_name, remote_ip, tunnel_type):
        ofport = br.add_tunnel_port(port_name,
                                    ...
                                    tunnel_type,
                                    self.vxlan_udp_port,
                                    self.dont_fragment)  #创建GRE|VXLAN等隧道
        ofport_int = -1
        try:
        ...
        br.add_flow(priority=1,
                    ...
                    actions="resubmit(,%s)" %
                            constants.TUN_TABLE[
                                tunnel_type])  #DeferredOVSBridge ： /usr/lib/python2.7/site-packages/neutron/agent/linux/ovs_lib.py(491)add_flow()
    ...
    def treat_devices_added_or_updated(self, devices, ovs_restarted):
        ...
        try:
            devices_details_list = self.plugin_rpc.get_devices_details_list(  #获取neutron-server数据库中的端点信息，对应的是网卡接口port，不是br-int上的port
                                                                              self.context,
                                                                              devices,
                                                                              self.agent_id,
                                                                              cfg.CONF.host)
        ...
        for details in devices_details_list:
            ...
            port = self.int_br.get_vif_port_by_id(device)  #获取本地br-int网桥下端点信息
            if not port:
    ...
    def treat_devices_removed(self, devices):
        ...
        #add by xm-2015.5.6
        self.qos_agent.clear_qos_rules(devices)
        ...
        try:
            for tunnel_type in self.tunnel_types:
                details = self.plugin_rpc.tunnel_sync(self.context,
                                                      self.local_ip,
                                                      tunnel_type)  #details={u'tunnels': [{u'ip_address': u'10.10.0.84'}, {u'ip_address': u'10.10.0.85'}, {u'ip_address': u'10.10.0.86'}, {u'ip_address': u'20.251.44.144'}]}
                if not self.l2_pop:
    ...
    def check_ovs_restart(self):
        ...
        canary_flow = self.int_br.dump_flows_for_table(
            constants.CANARY_TABLE)  #canary_flow=' cookie=0x0, duration=2778.097s, table=23, n_packets=0, n_bytes=0, idle_age=2778, priority=0 actions=drop'
        return not canary_flow  #当canary_flow=''空时，此处返回True;当canary_flow不为空时，此处返回False
    ...
    def rpc_loop(self, polling_manager=None):
        ...
        while self.run_daemon_loop:
            ...
            if ovs_restarted:  #当重启ovs-agent服务时，执行如下
                self.setup_integration_br()
                self.setup_physical_bridges(self.bridge_mappings)
                if self.enable_tunneling:
