...
class HaproxyNSDriver(agent_device_driver.AgentDeviceDriver):
    ...
    def create(self, logical_config):
        ...
        #创建配置文件并启动haproxy进程 ip netns exec qlbaas-3195b1f3-a453-4c35-a4a2-aa63b2678765 haproxy -f /var/lib/neutron/lbaas/3195b1f3-a453-4c35-a4a2-aa63b2678765/conf -p /var/lib/neutron/lbaas/3195b1f3-a453-4c35-a4a2-aa63b2678765/pid
    ...
    #add by xm-2015.7.24
    def undeploy_instance_without_synchronized(self, pool_id, cleanup_namespace=False):
        #LOG.info('xm212 updeploy_instance')
        namespace = get_ns_name(pool_id)
        ns = ip_lib.IPWrapper(self.root_helper, namespace)
        pid_path = self._get_state_file_path(pool_id, 'pid')
        ...
        if gw_ip:   # 设置默认网关IP
            ...
            if gratuitous_arp > 0:
                for ip in port['fixed_ips']:
                    ...
                    #ip netns exec qlbaas-3195b1f3-a453-4c35-a4a2-aa63b2678765 arping -U -I tap88837aa4-c3 -c 3 11.11.11.11
    ...
    # @n_utils.synchronized('haproxy-driver')  #usr/lib/python2.7/site-packages/neutron/openstack/common/lockutils.py(243)inner()
    ...
    #              constants.ACTIVE_PENDING_STATUSES) or                #constants.ACTIVE_PENDING_STATUSES = (ACTIVE,PENDING_CREATE,PENDING_UPDATE)
    ...
    #         return
    #
    #     if self.exists(logical_config['pool']['id']):   #存在则更新
    #         self.update(logical_config)
    #     else:
    #         self.create(logical_config)   #不存在则创建

    #usr/lib/python2.7/site-packages/neutron/openstack/common/lockutils.py(243)inner()
    @n_utils.synchronized('haproxy-driver')
    def deploy_instance(self, logical_config):
        #LOG.info('xm8 ssss')
        ...
        #存在则更新
            ...
            if (not logical_config or
                ...
                (logical_config['vip']['status'] not in
                 constants.ACTIVE_PENDING_STATUSES) or
                 #constants.ACTIVE_PENDING_STATUSES = (ACTIVE,PENDING_CREATE,PENDING_UPDATE)
                not logical_config['vip']['admin_state_up'] or
                (logical_config['pool']['status'] not in
                 constants.ACTIVE_PENDING_STATUSES) or
                not logical_config['pool']['admin_state_up']):
                #LOG.info('xm10 33')
                #pdb.set_trace()
                #self = <neutron.services.loadbalancer.drivers.haproxy.namespace_driver.HaproxyNSDriver object at 0x3e49890>
                #/usr/lib/python2.7/site-packages/neutron/openstack/common/lockutils.py(243)inner()
                self.undeploy_instance_without_synchronized(logical_config['pool']['id'])
            else:
                self.update(logical_config)
                #LOG.info('xm111 ssss')
        else:
    ...
    def _refresh_device(self, pool_id):
        #LOG.info(_('xm6 %s'),pool_id)
        logical_config = self.plugin_rpc.get_logical_device(pool_id)
        #/usr/lib/python2.7/site-packages/neutron/services/loadbalancer/agent/agent_api.py(51)get_logical_device()
        #LOG.info(_('xm7 %s'),logical_config)
        self.deploy_instance(logical_config)
        #logical_conf={u'vip': {u'status': u'PENDING_CREATE', u'status_description': None, u'protocol': u'HTTP', u'description': u'ddd', u'admin_state_up': True, u'subnet_id': u'bccff28b-31d0-41c9-a90d-55b2ffd501e0', u'tenant_id': u'8bcbc26515234902a12a0185b2a64bdd', u'connection_limit': 20, u'pool_id': u'3195b1f3-a453-4c35-a4a2-aa63b2678765', u'session_persistence': None, u'port': {u'status': u'DOWN', u'binding:host_id': u'', u'name': u'vip-e4fb4602-2d34-4ad9-9db0-3f3f3328ed17', u'allowed_address_pairs': [], u'admin_state_up': False, u'network_id': u'28710dbf-f0bd-417e-bbe2-2268d62ec26e', u'tenant_id': u'8bcbc26515234902a12a0185b2a64bdd', u'extra_dhcp_opts': [], u'binding:vif_details': {}, u'binding:vif_type': u'unbound', u'device_owner': u'', u'mac_address': u'fa:16:3e:f5:6d:56', u'binding:profile': {}, u'binding:vnic_type': u'normal', u'fixed_ips': [{u'subnet_id': u'bccff28b-31d0-41c9-a90d-55b2ffd501e0', u'subnet': {u'name': u'11.11.11.0/24', u'enable_dhcp': True, u'network_id': u'28710dbf-f0bd-417e-bbe2-2268d62ec26e', u'tenant_id': u'8bcbc26515234902a12a0185b2a64bdd', u'dns_nameservers': [], u'gateway_ip': u'11.11.11.1', u'ipv6_ra_mode': None, u'allocation_pools': [{u'start': u'11.11.11.2', u'end': u'11.11.11.254'}], u'host_routes': [], u'shared': False, u'ip_version': 4, u'ipv6_address_mode': None, u'cidr': u'11.11.11.0/24', u'id': u'bccff28b-31d0-41c9-a90d-55b2ffd501e0'}, u'ip_address': u'11.11.11.11'}], u'id': u'88837aa4-c391-4b02-97e8-d3e00746b7fe', u'security_groups': [u'edab0246-d00e-4014-864c-35a26187bbee'], u'device_id': u''}, u'address': u'11.11.11.11', u'protocol_port': 80, u'port_id': u'88837aa4-c391-4b02-97e8-d3e00746b7fe', u'id': u'e4fb4602-2d34-4ad9-9db0-3f3f3328ed17', u'name': u'vip'}, u'driver': u'haproxy_ns', u'pool': {u'status': u'ACTIVE', u'lb_method': u'ROUND_ROBIN', u'protocol': u'HTTP', u'description': u'', u'health_monitors': [], u'subnet_id': u'bccff28b-31d0-41c9-a90d-55b2ffd501e0', u'tenant_id': u'8bcbc26515234902a12a0185b2a64bdd', u'admin_state_up': True, u'name': u'test-pool', u'health_monitors_status': [], u'members': [], u'provider': u'haproxy', u'status_description': None, u'id': u'3195b1f3-a453-4c35-a4a2-aa63b2678765', u'vip_id': u'e4fb4602-2d34-4ad9-9db0-3f3f3328ed17'}, u'members': [], u'healthmonitors': []}
