...
class LoadBalancerPlugin(ldb.LoadBalancerPluginDb,
    ...
    """
    #add by xm-2015.5.26-for page of get_pools/vips/members/health_monitors
    ...
    def create_vip(self, context, vip):
        ...
        v = super(LoadBalancerPlugin, self).create_vip(context, vip)   ## DB中创建vip对象，该行代码执行完后数据库即有记录,其中status=PENDING_CREATE
    ...
    def create_pool(self, context, pool):
        ...
        p = super(LoadBalancerPlugin, self).create_pool(context, pool)  ## DB中创建pool对象，该行代码执行完后数据库即有记录,其中status=PENDING_CREATE
        ...
        driver = self.drivers[provider_name]   #service_provider=LOADBALANCER:Haproxy:neutron.services.loadbalancer.drivers.haproxy.plugin_driver.HaproxyOnHostPluginDriver:default
        try:
            driver.create_pool(context, p)  #neutron/services/loadbalancer/drivers/common/agent_driver_base.py line372 create_pool
    ...
    def update_pool(self, context, id, pool):
        ...
        #LOG.info(_('xm1 %s'), p.__dict__)
        driver = self._get_driver_for_provider(p['provider'])
        #LOG.info(_('xm2 %s'), driver)
